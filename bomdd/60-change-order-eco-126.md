# ECO-126 — 確認ダイアログ(ConfirmDialog)の CMP-011/L2 準拠是正 — 中央揃え漏れ+テーマ既定グレー+「はい/いいえ」の動詞化(REG-C5)(applied)
...
- **golden approved(maintainer 実機・基準 6 点・ja/en)**: §8 のとおり全点確認。基準 4 は golden 中に
  経路を整理して確認= 画面下の黒バー(SaveBar= ECO-102/103 のモードレス常駐)はスコープ外・
  対象は ✕ 押下時のモーダル(ECO-103 の OS 確認ダイアログ)で「戻る/破棄して終了」を確認。
- **M4**: 不要(共有部品の視覚/ラベル準拠是正・「確認して bool を返す」機能契約は不変)。
- **再発防止**: CP-UI-G6 刻印(潜伏実績つき)+CP-REGISTRY-LINT-122 検査C(クラス無し Button 台帳)+
  **confirmLabel 必須引数= REG-C5 違反の型強制**+GfConfirmDialogVisualParityTests 5 本。
- **教訓(read-across 明記)**:
  1. **共通言語の確立と legacy 面への遡及掃射は別の仕事**(ECO-117 の対話面版= N=2)。L2 は
     ECO-073 で確立・3 面で golden 承認済みだったが、最古参の ConfirmDialog(Phase 4 初期製造)には
     約 1 ヶ月適用されないままだった。CAD 側の対処(03 マトリクスへの legacy 行 lazy 遡及+CMP-011
     部品化+「生 Button 直置き禁止」制約)と VP2 側の lint 検査C が対で規約化(掃射手段の紐づけ)。
  2. **語彙の禁止則は型で強制できるなら型で強制する**。REG-C5「はい/いいえ禁止」は注意書きでなく
     `confirmLabel` 必須引数化で構造的に再発不能にした(ECO-124 型の「谷間を構造で塞ぐ」の
     API 署名版)。旧署名は「ラベル引数が無い= 構造的にはい/いいえしか出せない」= 逆向きの実証。
  3. **裁定文言の退化は共有部品の表現力不足が原因のことがある**。ECO-103 は「破棄して終了/戻る」を
     裁定していたが、当時の ConfirmDialog にラベル指定がなくはい/いいえへ退化していた= 裁定の
     完全な実現が本 ECO まで持ち越された(裁定を受ける側の部品契約も検収対象= ECO-121
     「照合先の昇格は正本の検収イベント」の対話部品版)。
  4. golden 中の経路確認(基準 4= SaveBar とダイアログの区別)は、**基準文に操作経路を書く**
     (ECO-104 教訓「golden 基準は実機到達可能な操作で書く」)の精密化= 同じ症状語(未保存確認)に
     複数 UI が応答する面では、基準に「どの UI が応答すべきか」まで書く。
