# Change Order — ECO-073(applied / クローズ済み 2026-07-13): コレクションの論理書き出し/取り込み(バックアップB層 V1)
...
6. **移動検出**(GF-073-08 で手順是正): 取り込み先コレクション B に、パッケージ記録と**異なる
   サブフォルダ**へ同一内容の画像ファイルを置いてスキャンしてから、A のパッケージを B へ取り込む →
   「移動を検出(自動追随)」に数えられ、付与が新パス(B 側)の画像へ着地する。
   (注: **同一コレクションへの再取込では移動検出は構造的に発火しない** — 移動後の再スキャンで
   旧行は missing として残り(新ファイルは規則 3a の pending 候補)、取り込みは id 一致が優先
   (§2.14.4)して「一致」に数え、タグは missing 行へ着地する。新パスへの引き継ぎは修復画面
   (ECO-005)の候補確定が担う=gate①裁定の設計どおり。)
...
13. B-2 で**ファイル選択後**、ファイルカードがグリフ+ファイル名太字+サイズ淡色の 2 行構成、
    パッケージ概要の作成日時が `yyyy/MM/dd HH:mm`・値が右寄せ太字+行区切り線であることを
    captures と並置確認する。
    (注: **未選択の初期状態は CAD 未定義**=グリフ+空白のカードは暫定。未選択状態の設計
...
## 14. golden所見 GF-073-06 の是正(2026-07-12 — 再機械受入)

### 14.1 所見と工程診断

- maintainer が §13.5(項目 15)の再実行で、長い待ちの後に**ウィザードが背面へ落ち**、例外で
  閉じたように見えた(実際は B-4 完了画面が背面に隠れていた。結果は正しい)。
- 工程診断(実測): `DatabaseManager.RunAsync` は work を**呼び出しスレッド上でインライン実行**し、
  `ApplyInTransaction`(同期・26 万行 INSERT)と `MatchImages`(同期走査)が **UI スレッドを
  数分間ブロック**していた。Windows は応答なしウィンドウをゴースト化するため、完了時に
  Z オーダーが乱れ B-4 が背面へ落ちる。真因=重い同期処理が UI スレッドへ到達する合成
  (エンジンは仕様どおり・VM 層の欠陥)。
- 併せて R3 記録(51-cheat-log): 適用中の進捗表示・キャンセル挙動は CAD 沈黙(mock に適用中
  progress なし・キャンセルはウィンドウ close のまま)— 本 GF では触らない。

### 14.2 先行probe(R5)

- `CpPackage073Tests` へ「プレビューと実行は呼び出しスレッドを塞がず制御を即時返す」を追加
  (2000 画像 fixture・ToPreview/Execute の Task が呼び出し直後に未完了であること+完了面へ到達)。
- 是正前実測: **1 件不合格**(プレビューが同期ブロック=制御が完了まで戻らない)。

### 14.3 是正diff

- `CollectionImportViewModel`: `PreviewAsync`/`ApplyAsync` の呼び出しを `Task.Run` で
  バックグラウンドへ(継続は await で UI スレッドに戻る)。エンジン・挙動・視覚は不変。

### 14.4 再機械受入

- `dotnet build`: 0 warning / 0 error。`ViewPrism2.Tests`: **641/641 pass**(probe 緑転)。
- `ViewPrism2.Oracle`: 109 pass / 2 known skip(R6 不変)。`validate_bom`: 0/0。判定は exe 直接実行。

### 14.5 gate②再操作(§13.5 項目 15 の再実施)
...
## 16. golden所見 GF-073-08 の是正(2026-07-12 — 検査手順の欠陥・doc是正+挙動固定)

### 16.1 所見と工程診断

- maintainer が §8.6 項目 6(移動検出)を「同一コレクションでファイル移動→再スキャン→再取込」で
  実施し、①再スキャン結果に移動先サブフォルダが見えない ②移動が検出されない、と報告。
- 工程診断(実測 2 本で固定): **製品挙動は仕様どおり・欠陥は golden 手順(検査器)側**。
  - 同一コレクション再取込では、移動後の DB は「旧行=missing+新パス=pending 候補(規則 3a)」で、
    取り込みの照合は **id 一致が最優先**(§2.14.4「一致=id またはパス+ハッシュ」・
    MatchImage 実装)→ 「一致」に数えタグは missing 行へ着地。新パスへの引き継ぎは修復画面
    (ECO-005)の候補確定が担う= gate①裁定の設計そのもの。**この経路で「移動を検出」は
    構造的に発火しない**ため、旧項目 6 の手順は検証不能な手順だった(検査手順の欠陥)。
  - 「移動を検出」の正しい発火経路= **id 不在の取り込み先**(別コレクション/別ライブラリ)で
    パッケージ記録と異なるパスに同一ハッシュが一意に存在する場合。実測テストで付与が新パスの
    行へ着地することを固定。
- ①(スキャンに test フォルダが出ない)の要因は 2 系: (a) スキャンの再帰は**コレクション設定
  `IncludeSubfolders` に従う**(ScanService.cs:120)— 設定が無効なら仕様どおり拾わない。
  (b) 有効でも移動先の新ファイルは**規則 3a の pending 候補**として登録され、既定一覧は
  status=normal のみ表示(INV-010)のため**画像タブには見えない**(修復画面には missing 行+
  候補として現れる)。いずれも実装欠陥ではない。maintainer 実機がどちらかは項目 6 再実施時に確認。

### 16.2 挙動固定(probe ではなく仕様の実測 pin)

- `CpPackage073Tests` へ 2 本追加(いずれも**初回から緑**=診断の裏取り):
  「同一コレクション再取込では移動ファイルは id 一致で『一致』に数えタグは missing 行へ着地する」
  「別コレクションで別パス同一ハッシュの画像は移動検出され付与が新パスへ着地する」。

### 16.3 是正diff(doc-only)

- §8.6 項目 6 を「別コレクション B に異なるサブフォルダで同一内容ファイルを配置→スキャン→取り込み」
  へ書き換え、同一コレクション再取込の挙動(一致+修復導線)を注記。製品コードは無変更。

### 16.4 再機械受入

- `dotnet build`: 0 warning / 0 error。`ViewPrism2.Tests`: **644/644 pass**。
- `ViewPrism2.Oracle`: 109 pass / 2 known skip(R6 不変)。`validate_bom`: 0/0。判定は exe 直接実行。

### 16.5 gate②再操作

- 項目 6 を**是正後の手順**で再実施する。同一コレクションの移動は(移動検出でなく)
  「一致」+修復画面での候補確定でタグが新パスへ引き継がれることを確認できれば十分
  (規則 3a=CP-PACKAGE-032 で unit 固定済み)。

## 17. クローズ(2026-07-13 gate②合格)

### 17.1 実機確認(maintainer・CP-UI-G13)

- **全 17 項目合格**: §8.6 の 1〜10(B-1 書き出し・冪等・missing 登録+過半警告・競合 4 択・互換 NG・
  移動検出=GF-073-08 是正後手順・鮮度・物理非破壊・V1 差分許容判定・ja/en)+ 11(§9.5)+
  12(§10.5)+13(§11.5)+14(§12.5)+15(§13.5)+16(§14.5)+17(§15.5)。
- GF-073-01〜08 の 8 所見(視覚転写 5・実装バグ 2・検査手順 1)をすべてプローブ先行で是正した上での承認。

### 17.2 再発防止(恒久化の所在)

- CP-UI-G13 characteristic へ潜伏実績つきで 4 観点を追記(ダイアログ共通言語+面別転写の captures
  全面突合/同一ライブラリ内取込+生 SQL 非露出/実行中 UI 応答/移動検出の正しい検査経路)。
- 機械側: `GfPackageVisualParityTests`(視覚 pin)・`CpPackage073Tests`(同一ライブラリ取込・
  照合意味論の挙動固定・スレッド非ブロック)で恒久封止。

### 17.3 教訓

- **視覚転写の水平展開は「同型面の列挙」を先にやる**: GF-01(B-1 カード)→02(callout)→
  03(B-2 カード)→04(stepper)→07(B-3 全面)と、同じ「mock 言語の転写漏れ」が面を変えて
  4 回連鎖した。是正のたびに「直した面」だけ probe を pin し、**同型要素を持つ残りの面を列挙して
  同時に検査対象化する**ことを怠ったのが構造要因(GF-072-01 の captures 同梱だけでは足りず、
  読み手が面単位で並置しない限り漏れる)。既存教訓「ダイアログ共通言語は共通契約で固定」
  (ECO-073 GF-01/02)の一般化=**共通言語は『言語』でなく『適用面の全列挙』とセットで初めて
  検査可能になる**。方法論昇格候補。
- **大量データの新入力は既存部品の検査面を再開させる**: ECO-073 の missing 参照登録(26 万行)が、
  既存の修復画面(ECO-005)と共有 DB ゲートの潜在非スケールを顕在化させた(ECO-075 へ分離)。
  新機能が既存部品へ「これまで無かった規模の入力」を供給する場合、その部品の計算量・スレッド前提を
  影響 BOM に含める。ECO-062(26 万件全走査)と同族= read-across。
- **検査手順自体も工程診断の対象**(GF-073-08): golden 不合格の原因が製品でなく手順(検査器)で
  あり得る。「プローブが不合格にならなければ診断が誤り」の裏面として、手順の検証可能性を
  fix 時に確かめる(移動検出は同一コレクション再取込では構造的に発火しない)。
