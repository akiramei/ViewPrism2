...
## 9. クローズ(2026-07-15 golden 合格・applied)

**gate② 合格(maintainer 実機)**: ja 起動の視覚不変・en 切替でデフォルト行/ヘッダー/移動先が
"Default" へ即追随(バッジと名前の言語一致=報告症状の解消)・ja 往復・回転降格の時刻名不変。

**M4 同期(accept 時)**: ①CP-I18N-010 へ**第 3 の様態**(永続データ焼き込み=lint の構造的盲点)
の検査次元+CpI18n095 を追補 ②CP-UI-G1 へ golden 観点(デフォルト名の切替追随・潜伏実績つき)
③E-UI-WORKSPACE-043 へ as-built 注記(表示時解決の挙動仕様)。

**再発防止**: CP-I18N-010 ⑤(機械)+CP-UI-G1(golden 両言語実施の既存規律に本観点を追加)。

**教訓**: **i18n 漏れは「文言がどこに立地するか」で様態が分かれ、検査はその立地ごとに要る** —
①XAML 直書き(ECO-079 第 1 層)②VM 算出(GF-079-01 第 2 層)に続き、本件で
③**永続データへの焼き込み(Core シード→DB→表示)**が確定した。静的 lint は①②しか見えず、
DB を経由した瞬間に盲点になる(「デフォルト」は grep すれば見えたのに、lint の走査対象が
src の表示層だけだった)。判定規準は**「その文字列は UI ロケールの関数か、ユーザーデータか」**
— ロケールの関数なら DB に置いた時点で欠陥(表示時解決へ)。ECO-080 control-plan の
「暗黙前提 3 様態」と対をなす i18n 立地 3 様態として read-across(既存シード経路の他事例=
今回の全数走査では workspace のみ。新規シード導入時はこの規準で審査)。副収穫=同じ再構築
配線で baked Sub ラベルの不追随も解消(切替再射影は行 VM 全体を対象にするのが正)。
