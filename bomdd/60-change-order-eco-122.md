# ECO-122 — UI 部品表(04_component_registry)適合性検査の配線 — lint 新設+視覚 probe の照合先移行(applied)
...
- **検収= golden n/a・機械証拠 4 点**(maintainer accept 指示): §8 のとおり。
- **M4**: 不要(tests のみ・src/CAD 無変更= as-built 乖離なし)。
- **再発防止**: **CP-REGISTRY-LINT-122 新設**(automated・33-control-plan)= 写像の配線契約+lint の
  tolerance(allowlist 外 0+死亡エントリ 0)+検出限界宣言を刻印。CP-UI-G1 の ECO-123 節を
  「写像参照へ移行済み」へ同期。
- **教訓(read-across 明記)**:
  1. **検査の照合先一元化は「宣言」でなく「配線+陽性対照」で成立する**。柱5(照合先は部品表)は
     決定済み方針だったが、写像を経由しない限り検査上の実体を持たない(二重定義のまま)。
     配線の生存証明= 写像を壊すと probe が赤転する実測(MenuWidthSort 999→2 本赤)。
     ECO-120「検査範囲は読んでいるファイル一覧でしか保証されない」の対偶=
     **読ませたいものは読む経路を作って壊して確かめる**(陽性対照の常設 N=5: 053/078/107/120/122)。
  2. **lint の first-run は契約と実装の全面突合イベント**として設計する。本 first-run は起票時予測と
     完全一致(新規違反ゼロ)だったが、R8 独立レビューが同じ突合を人力で広げて CMP-004 セグメント
     契約の抵触・並び替え Padding/BoxShadow の未記帳乖離を検出した= 機械 lint のトランシェ外は
     人力レビューが埋める(検出限界の宣言とセットで初めて被覆が言える= ECO-107 教訓1 の運用形)。
  3. **写像の登載基準は「値の一致」でなく「トークン所掌の一致」**。同値の色(#F4F6FA 等)が別意味で
     複数所掌に現れるため、値ベースの機械写像は誤写像を作る(CMP-010 裁定「意味が異なれば同型でも
     別部品」の写像版= ECO-121 教訓1「被覆検査」と対をなす「誤結合検査」)。
