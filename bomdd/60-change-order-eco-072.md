# Change Order — ECO-072(staged / gate①裁定済み): カタログDBスナップショットの作成・検証・時点復元(バックアップA層)
...
2. **SS-001(UI 入口)の裁定**: 本ダイアログ群をどのメニュー/タブから開くか(mock 未設計・CAD
   review_points 登録済み)。`/eco-fix ECO-072` 着手時までに maintainer が裁定する。
3. `/eco-fix ECO-072` で probe(§3.2)→実装→機械受入。
4. gate② golden: 作成・検証・復元・失敗フォールバックの実機確認。
