# Change Order — ECO-051(applied): V3 旧 UI(SimilarSearchWindow/MergeDialog/TrashView)一式の撤去 — ECO-024 原典撤去の撤去漏れ残骸
...
- **maintainer 承認**: 到達不能残骸の撤去=視覚不変につき、機械受入全緑+L1 スモーク+軽量確認で受入。
- **再発防止**: M-UI-SIMILARITY-023 の tombstone(継承先・移行記録・残乖離を trace_status に恒久記録)+
  受入観点対応表(§7)を移行判断の証跡として ECO 本文に保存。CP 追記なし
  (撤去手順の教訓は golden 観測点の性質でない — doc-only 系前例と同扱い)。
- **教訓(一般形・昇格候補)**: **撤去の依存検証は「型の直接参照」でなく「呼び出しグラフの閉包」で行う** —
  ECO-024 は legacy の直接参照を消したが、サービス境界(IWindowService)越しに繋がる旧 UI 一式が
  検証から漏れ、9 ファイル+3 メンバが 5 日間残骸として潜伏した。インターフェースは参照を「生きている
  ように見せる」ため、public API は宣言の存在でなく**呼び出し元の有無**で生死を判定する。
  残骸の実害は実測済み: 診断ノイズ(ECO-050 の「三者三様」の 1 つが死者)・偽装備(仕様適合値 70 が
  死者にのみ存在)・台帳乖離(M-BOM が死んだ artifact を指す)— 動かないコードにも保守コストが発生する。
  read-across: ECO-048 教訓「拡張は凍結面の棚卸しから」と対をなす「撤去は到達閉包の棚卸しから」。
  副産物として REQ-067 プレビュー不在(死者にのみ実装が残っていた機能欠落)を発見= 残骸は欠陥も隠す。
