# ECO-131 — クロスタブ状態不整合 — 作業タブの状態変更が画像タブ母集合を無効化しない(applied)
...
## 10. クローズ(2026-07-22・golden 合格・ECO-128 と合同)

- **実機確認(maintainer・ECO-128 と合同)**: ①作業タブで削除→画像タブへ切替でその画像が一覧から消える
  ②作業タブで復元→画像タブへ切替で未裁定バッジ付き(pending)で現れ、⋯「未裁定の画像」ダイアログと
  グリッドが一致(今回の GF-128-01 症状の解消)③大きいコレクションで作業タブ→画像タブ復帰直後に別コレクションへ
  素早く切替えても前コレクション母集合が混入しない(世代ガード)④回帰(画像タブ単独のゴミ箱削除/復元・裁定・
  タグタブ変更反映・スキャン)を承認。
- **再発防止**: CP-UI-G1 へクロスタブ鮮度観点+**「既存の内部再読込を対話中の fire-and-forget 経路へ転用すると、
  モーダル閉後前提の無ガードが競合欠陥に化ける(世代ガードの read-across)」**を刻印。機械 pin=
  CpUiG1CrossTabRefreshTests+CpUiG1CollectionScopeTests(TCS ゲートの世代ガード probe)。
- **M4 同期**: 不要 — VM 内部のコヒーレンシ是正(MainWindowViewModel シェル配線+ImageTabViewModel の
  RefreshContentAsync/ReloadImagesAsync ガード)で as-built 乖離なし(ECO-038 型)。
- **教訓**(read-across 明記):
  1. **「二重管理」に見える不整合の多くは単一真実+キャッシュ鮮度の問題** — 本件は images.status 単一列
     (DB=真実)に対し、画像タブと作業タブが独立にインメモリ母集合をキャッシュし、片方向(作業タブ→画像タブ)の
     無効化が構造的に欠けていた。診断はまず「状態が二重に格納されているか」と「キャッシュが無効化されないか」を
     切り分ける(⋯ダイアログの DB 直読みが単一真実の証拠になった)。
  2. **既存の内部再読込 API を新しい呼び出し文脈へ転用するときは、その API が暗黙に依存していた前提を棚卸しする** —
     `ReloadImagesAsync` は従来モーダル閉後(非対話=コレクション切替と競合しない)だけから呼ばれており世代ガードが
     不要だった。ECO-131 が対話中シェルから fire-and-forget で呼ぶ経路を開いたことで、await 中のコレクション切替と
     競合する新たな穴が開いた。**呼び出し文脈の前提(非対話/モーダル境界)は API の暗黙契約であり、転用時に
     read-across する**(GF-129-01「母集合取得サイト拡張の全経路 read-across」の呼び出し文脈版・BomDD 昇格候補)。
