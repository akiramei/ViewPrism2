# Change Order — ECO-045(applied): TAG-008 裁定の取り込み — 使用中タグ定義の削除は拒否(無効化/非推奨化は別操作)
...
### 実機確認(maintainer・基準 1-3 全合格)

①画像に付与済みのタグの削除=拒否理由表示・タグと付与は無傷 ②ビュー階層に配置しただけの
タグ=拒否・配置無傷 ③未使用タグ=削除成功。基準 4(子ルート化)は実機対象外
(§7 の修正どおり機械受入 S-38+CP-TAG-011 で担保)。

ウォークスルー中の**別所見**(ビューにタグを配置→未保存のまま削除→保存で「予期しないエラー」)は
実測診断の結果 **ECO-045 起因でない潜伏欠陥**(保存経路・dirty ガードとも本 ECO 無改変。
ECO-045 のガードは DB 参照のみを見るため未保存編集状態は関知できない設計どおりの範囲)と確定し、
**ECO-046 へ R3 分離起票**(`1a996bc`)。

### 再発防止(CP 明記)

- **CP-UI-G6**: 使用中削除拒否の観点(拒否理由表示・無傷・未使用は削除成功)+
  「golden 基準は実機到達可能な操作で書く」実績を明記。
- **CP-TAG-011**: 使用中削除拒否ベクタ(3 参照種別+子のみ+未使用)を test_vectors へ追加。
- M4: 20-spec §2.2(fix 内)・M-VIEWSVC-012 interface_contract.tags(Delete 拒否規則)。

### 教訓

**DB を正とするガードは、未コミットの編集状態(dirty エディタ)を保護しない。** 「使用中は拒否」の
ような参照整合ガードを導入したら、その参照が**まだ DB に無い場所**(未保存の編集ツリー・
モーダルの作業中状態)との谷間を必ず golden 手順に含める — 今回はその谷間が ECO-046
(保存時 FK 違反の未処理例外)として即日顕在化した。read-across: ECO-041(未配線 UI =
golden の谷間)・ECO-044(見せかけトグル)に続く「検査が届かない面」系の第 3 形態
= **ガードの参照面(DB)と操作の参照面(メモリ)の不一致**。また、golden 基準の策定時は
各基準の操作経路が実機に存在するかを先に検証する(基準 4 は Core 機能に UI 露出が無く
到達不能だった — maintainer の指摘で判明)。
