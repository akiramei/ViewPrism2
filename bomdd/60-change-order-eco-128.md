...
- **gate①(裁定)— 必要**:
  1. **実施順序の承認**: ECO-130(二段階)→ ECO-129(pending 意味論)→ 本 ECO の順を推奨
     (§3 事実 4 の依存)。単独先行は不可視化+タグ損失リスク。
  2. **S-29 固定オラクル行の処置**(R6 衝突): (i) 旧行を skip 化し理由刻印+新意味論行を追加 /
     (ii) 意味論改訂を伴うオラクル行の廃止手続きを新設(BomDD 方法論側と整合)/
     (iii) その他。※ ECO-129 の OC-5 衝突と同一論点のため一括裁定を推奨。
