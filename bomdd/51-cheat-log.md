...
| F-3 | Run2 | ルート消失時の一括 missing 化保護(フォルダ自体が読めない場合は中断) | **accept** — 妥当な防御(USB 取り外しで全画像 missing 化する事故の防止)。仕様 §2.1 へ次版で転記。**ECO-135(2026-07-22)で refine: 中断コードを汎用 IoError→専用 ScanRootMissing 化=失敗理由の識別性を回復(一括 missing 化しない防御自体は不変)** |
...
## 2026-07-21(ECO-129 R8 スコープ外所見)

- **初回スキャンの中断・例外でも last_scan が更新され、残余が次回「再スキャン=pending」になる崖**
  (R8 所見11): ScanService の finally 無条件 UpdateLastScanAsync は REQ-015 仕様どおりだが、
  v5.0 では初回取込の残り全件が裁定対象になる(「初回のみ normal=登録行為が裁定」の趣旨と乖離)。
  仕様側の残余リスク=症状観測(初回中断→大量 pending)で分離起票。候補是正= 初回スキャンの
  中断時は last_scan を更新しない(isInitialScan 分岐)= REQ-015 の改訂を伴う。
- **ScanStaging.PendingTotal の文言境界**(R8 所見12): ContentChanged は pending 起点のメタ追随を
  含むため「適用後、N 件が裁定対象になります」が既 pending を含み得る(新たに裁定対象になる数では
  ない)。情報表示のみ・安全側。厳密化するなら pending 起点メタ追随の独立カウントが必要。

## 2026-07-21(ECO-129 gate② GF-129-01 是正中のスコープ外観測)

- **CpUiG6SaveBarTests「保存バーとトーストはdirty時の…」が単発 flake**(ObjectDisposedException
  'TestContext'・Avalonia Headless DispatchCore 経由・CpUiG6SaveBarTests.cs:244): 同一 dll 内の
  直後再実行では緑=非決定性。検査器ライフサイクル系(E15/E19・ECO-082 保留静置の SQLite Dispose×
  background 残タスク族と同カテゴリ疑い)。N 蓄積で検査器ライフサイクル一括起票候補へ合流。

## 2026-07-21(ECO-128 是正中のスコープ外所見=R8 独立レビュー由来)

- **作業タブに pending(未裁定)導線がない=復元 pending が作業タブから不可視**(WorkTabViewModel.cs:1404
  RestoreSelectedTrash): ECO-128 で復元→pending 化した結果、作業タブは normal 限定(INV-W2/INV-010 v5.0)
  のため復元画像が作業タブ一覧から消え、そこで裁定もできない(裁定は画像タブの ⋯「未裁定の画像」でのみ)。
  workspace_images 所属は不変・画像タブ経由で裁定可能=データ喪失はないが、作業タブ完結の UX が切れる。
  是正候補= 作業タブにも pending バッジ+裁定導線(ただし pending はコレクション単位・作業スペースは
  横断しうるため設計判断を要する=INV-010 v5.0 の消費サイト別規定の残件)。**別 ECO 候補**(ECO-128 の
  gate① スコープ=src 1 行+spec+tests の外・maintainer 設計まとめ §6 は復元遷移のみ規定)。
- **RestoreStatusAsync に deleted 限定 WHERE がない**(ImageRepository.cs:197): AdjudicatePendingAsync は
  `WHERE status='pending'` で遷移を原子強制するが、RestoreStatusAsync は id のみで無条件更新し deleted 限定は
  TrashService の read-then-write(TOCTOU 窓)に依存。置換前の UpdateStatusAsync と同一の非原子パターン=
  退行ではない。単一ユーザーのデスクトップ+deleted 行はスキャンで status 遷移されないため実害極小。
  是正候補= `WHERE id=@Id AND status='deleted'`(AdjudicatePending と対称)。防御的・別 ECO/棚卸し候補。
- **作業タブ復元後にトラッシュバッジ件数が未更新(疑い・先在)**(WorkTabViewModel.cs:1408
  RestoreSelectedTrash): Purge/Empty は RefreshTrashCountAsync を呼ぶが Restore は呼ばない(画像タブ側
  ImageTabTrashViewModel:151 は呼ぶ=面間非対称)。復元後に _trashCount 依存のバッジが stale になり得る。
  ECO-128 は本体未変更(コメントのみ)=先在事象の疑い。要実機確認→症状観測で分離起票。

## 2026-07-21(ECO-131 R8 独立レビューのスコープ外所見)

- **クロスタブ母集合再読込が「作業タブ訪問」単位で無条件**(MainWindowViewModel.cs:178
  OnSelectedTabIndexChanged value==2): ECO-131 は作業タブ入場で content-stale を立て復帰時に
  RefreshContentAsync(全母集合再読込)する。変更有無に関わらず「作業タブを覗いて戻る」毎に
  ReloadImagesAsync+BuildEntries+Recompute が走る=26 万件級で往復毎の全再読込コスト。より精密な
  前例=タグ側は TagsTab.DataChanged イベント駆動で stale 化(MainWindowViewModel.cs:63)。作業タブにも
  StatusChanged 通知を持たせれば精密化できるが、入場ベースは「イベント取りこぼし不能」で堅牢という
  対抗利点あり=設計トレードオフ。症状観測(26 万件でのタブ往復レイテンシ)で分離起票候補。
- **view 軸で母集合減少時の 0 件 value ノード残存(先在共有特性)**(ImageTabViewModel.cs Recompute の
  view 分岐): RefreshContentAsync は ReloadImagesAsync+Recompute のみで ReloadViewGraphAsync(graph
  再構築)を呼ばない=最後の裏付け画像を失った value ノードが full reload なら消えるところ Recompute
  では _viewRoot に残り 0 件チップとして残存し得る。ただしこれは OpenPendingReview/OpenRepair と
  完全同一の先在特性で ECO-131 固有ではない(出荷済み挙動)。処置するなら別 ECO。

## 2026-07-22(ECO-133 R8 独立レビューのスコープ外所見)

- **【中・別 ECO 候補】部分適用失敗→破棄で「DB 完全無変更」と誤報+画像タブ非更新**
  (FolderManagementViewModel.cs:269-285): ApplyStagedAsync 失敗時 VM は Outcome を触らない(失敗設定は
  StageAsync 経路のみ・Apply 失敗経路は StatusMessage のみ)。ユーザーが再スキャン誘導どおり破棄すると
  Outcome=Discarded(Applied=false・Error=null)で返り、呼び出し側は folder.scanDiscarded(=「DB 完全
  無変更」)を表示し outcome.Applied 偽のため DataChanged 非発火=画像タブを再読込しない。**しかし部分適用
  で DB は実変更済み**=(a)メッセージが事実に反し(b)画像タブが部分適用済み DB に対して stale。次回
  再スキャンで収束するがそれまで露出。**ECO-133 以前から存在**(Apply 失敗全般の Outcome 意味論)だが、
  ECO-133 の回復経路「閉じて再スキャン」がこの誤報経路を常用ルート化=露出頻度を上げる。是正候補(別 ECO)=
  Apply 部分失敗を Outcome へ「部分適用」種別として載せ破棄時も DataChanged 発火/メッセージを「一部適用済み・
  再スキャン要」へ。last_scan は全バッチ成功後のみ更新(ScanService.cs:446)=据え置きは活かせる。
- **【低・UX nit】失敗後も ApplyLabel が「{count}件の変更を適用」のまま**(ScanSummaryViewModel.cs:415):
  ApplyLabel は BuildSummary 時の値のまま(CanApply 変化で再計算されない)。失敗後、無効化ボタンに件数
  ラベルが残る。StatusMessage が封鎖理由を明示するため実害低いが、無効ボタン+件数ラベル併置はやや誤解。
  任意改善(必須ではない)。

## 2026-07-24(ECO-140 R8 独立レビューのスコープ外所見)

- **【中・DB 基盤の分離候補】Microsoft.Data.Sqlite は実行開始後の query cancellation を保証しない**:
  ECO-140 新規の pending∪missing 母集合/件数/タグ query は `CommandDefinition(cancellationToken)` まで
  token を伝播したが、同梱 provider の長時間 SQL 実測では `CancelAfter` 後も同期実行が完了し、
  in-flight interrupt にはならなかった。REQ-103/R8 残件③の対象である「現在ファイルの SHA-256」は
  `HashDataAsync(stream, ct)` で閉扉中断済みで、Window close 自体も DB 完了を待たず stale 結果を破棄するため
  本 ECO の裁定意味論は満たす。SQLite native interrupt は単一共有接続+SemaphoreSlim の全 repository に
  波及し、適用系 transaction の非中断契約とも衝突し得るため混ぜない。分離時は専用読取接続/operation 世代を
  含む DB 基盤設計と、実 provider の長時間 query を CancelAfter して早期終了する統合プローブを対で起票する。

## 2026-07-24(ECO-140 golden 中のスコープ外所見)

- **【低〜中・別途是正候補】⋯メニュー既存行「Backup & transfer in Settings…」(view.backupTransferGuide)の
  en 截断**: CMP-006 幅 208 でラベル実効幅を超え「Backup & transfer in Set」と語中截断(GF-084-01
