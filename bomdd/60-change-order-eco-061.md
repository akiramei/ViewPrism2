# ECO-061 (applied) — ECO ライフサイクル遷移の状態不変条件検査 — validator 検査次元の追加+eco-accept post-condition
...
## §8 クローズ(2026-07-11・gate② 合格)

### 8.1 golden 結果(2026-07-11・maintainer 実機)

maintainer が ViewPrism2 リポ直下で gate② 3 点を実行し、全て合格を確認:

1. `python bomdd/validate_bom.py --selftest` → **selftest: OK**(変異 5 種のうち純粋層 4 種:
   fixなし=E15/E16・順序逆転=E17・参照先不在=E18・飛び越し/逆行=E19)
2. `python bomdd/validate_bom.py --selftest-lifecycle` → **selftest-lifecycle: OK**(実 git 統合:
   正常 fix→accept・**別系統ブランチ**=E15・順序逆転の実 DAG=E17)
3. `python bomdd/validate_bom.py` → **0 error / 0 warning**(既存 60 ECO への遡及検査初回 —
   除外は lifecycle_evidence.legacy の宣言のみ・黙殺なし)

### 8.2 再発防止(恒久回帰の所在)

製品 CP への観点追記は**非該当**(製品コード・製品挙動とも不変)。本 ECO の恒久回帰は
**検査器自身**が担う: `--selftest` の lifecycle 変異(検査が消えれば「lifecycle 検査が存在しない」で
FAIL する先行赤の恒久化を含む)+`--selftest-lifecycle` の実 git 統合+`hooks/commit-msg` の
コミット時強制。pre-commit が selftest を毎コミット実行するため、検査次元の無音死(ECO-053 様式)も
検出される。「ECO の是正は自己適合ゲートへ陽性対照つき恒久回帰として収載して閉じる」
(BomDD playbook §13・harness ECO-001〜005 の教訓)の製品リポ初適用。

### 8.3 教訓

- **工程の状態台帳も検査対象の状態機械である**。台帳上の 0 エラーと工程実態の乖離は、構造整合検査では
  捕捉できず、**状態不変条件**(履歴証拠の実在・祖先関係・遷移順序・遷移エッジ)の検査次元でのみ塞がる。
  散文前提と入口スキルは「起動された場合の」順序しか強制できない(ECO-060 違反の実測)。
  read-across: ECO-053(検査器の素通し様式→selftest 陽性対照)の系譜に「検査次元そのものの不在」を追加。
- **自己参照制約は「証拠を遷移コミット自身に携行させる」ことで解消する**(案 a)。台帳に SHA を書く
  設計(案 c)は二段階状態を生み、tag(案 b)は push 忘れの新たな無音面を生む — 証拠は遷移と同一の
  原子的イベントに載せるのが最小。
- **pre-commit で検査できない面は commit-msg で塞ぐ**。フック面の選択は「その時点で証拠が観測可能か」
  で決まる(pre-commit=履歴+作業ツリー / commit-msg=+メッセージ)。
- 一般形は方法論へ**還元済み(本 ECO に先行)**: BomDD FINDINGS §11.3(継続運用レジーム初観測)・
  playbook §13(fail-closed 防御は④に置く)・silence-checklist §12(台帳自身も状態機械)。
