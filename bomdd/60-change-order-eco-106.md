# Change Order — ECO-106(applied): タグパレットの常駐メッセージが言語切替へ追随しない(ECO-104 golden 中の実機所見)
...
**maintainer 実機承認(基準 1 点)**: タグをビュー階層へ配置(未保存)→ パレットで削除試行 →
拒否メッセージ表示 → 言語 ja↔en 切替でメッセージが即追随 — 合格(所見スクリーンショットの
再現手順そのままで検収)。

**再発防止の場所**: CP-UI-G6(常駐メッセージのロケール追随観点+脆弱クラス定義=「次のユーザー操作
まで再解決機会がない表示」)/ CpUiG6HierarchyEditorTests(実アセット ja↔en 往復=恒久ガード)/
51-cheat-log(lint 第 3 次元「解決タイミング」記帳= ECO-106 起票時)。

**教訓**(1 段落・既存教訓との関係):

**同一アンチパターンの複数面顕在化は、個別是正と並行して「脆弱クラスの定義」を残す** — 本件は
ECO-104 1.2(保存バー)と同じ「Resolve 済み文字列の保持」だが、全数調査(約 15 VM・50 箇所超)を
悉皆是正せず、実害条件で層別して「常駐メッセージ(メインタブ面に残り、次のユーザー操作まで再解決
機会がない表示)」を脆弱クラスとして CP と lint 候補に定義した。モーダル内の一時表示・状態更新で
再計算されるラベルは実害が薄く、一律の機械是正はノイズになる。既存教訓との関係= ECO-095「値の
権威主体と再導出可能性」の適用範囲を「永続データ→VM 一時状態」へ外延した ECO-104 の続きで、
外延先の**全数分布と危険度の層別**まで行った点が増分(i18n lint 拡充 ECO の設計入力=検査 3 次元
〔重複キー・未使用キー・解決タイミング〕が揃った)。

**スコープ外の後続**: ECO-105(撮影ハーネス・staged・未着手)・i18n lint 拡充 ECO(3 次元の証拠が
揃い起票候補濃厚)。
