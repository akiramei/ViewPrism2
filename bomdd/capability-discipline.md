...
### A/B の前提条件 — 同等性契約のオラクル分離(着手前に必須)
性能 A/B を始める前に、**横断ゲートと工場固有を分離**する。本プロジェクトの具体例(着手前に解消すべき自己矛盾):
- CPOL-103: pHash は preserve_with_adapter(ビット一致不要・検索体験の同等で可)
- だが S-19(単色=0x8000…)/S-21(特定検索結果)は **"このビルドの pHash" を凍結**している
- → factory-B(早期縮小等)が CPOL-103 的に等価でも S-19/S-21 は落ちる。**A/B 前にオラクルを分割**:
  - **(a) 横断正しさ = 検索順位/距離の同等**(全工場共通ゲート。"ranking/distance equivalence oracle")
  - **(b) このビルドの pHash 値**(工場固有・横断ゲートにしない)
