...
- **golden**: approved(2026-07-19 maintainer 実機・26 万件ビュー基準 5 点): ①タグ編集モードの
  クリック/Ctrl クリックが即時 ②ファイル操作モードも同様(コピー/場所を開くは従来どおり)
  ③SHIFT 範囲は従来と同一挙動(表示順・歯抜けなし) ④意味論退行なし(タグ付与パネル・整理マーカー・
  IMG-025 縮退) ⑤作業タブも同様。
- **再発防止**: CP-UI-G1 へ計算量契約+潜伏実績+検出限界の宣言を刻印。E-UI-BROWSE-022 へ
  invariant「選択操作のコストは選択集合サイズに比例(母集合サイズ非比例)」を追加(M4)。
  機械側= 構造 probe 3 本(計器 ContextEnumerationCount)+スキャン段階公開×entry 解決 probe で pin。
- **M4 同期**: E-BOM invariant のみ(spec/M-BOM/CP 以外は変更なし=視覚・意味論不変の計算量是正。
  REQ 新設なし=ECO-038 型の実装層是正)。
- **残課題**: WorkTab BuildContextPanels の O(スペース内件数)残余(§7.4 所見 3=規模前提つき許容)・
  SaveBar flaky 4 例目(51-cheat-log・一括起票候補)。

### 教訓

1. **規模系 NFR は経路ごとに独立して破れる**: 26 万件級の応答性は ECO-064(起動)・ECO-062
   (類似検索)で 2 度是正済みだったが、選択クリック経路は両スコープ外で無傷のまま残った。
   「ある規模での顕在化を 1 経路直しても、同じ規模に触る他経路は別の欠陥として潜伏し続ける」—
   大規模 fixture での顕在化時は、同規模に触る**操作経路の棚卸し**(クリック/ホバー/スクロール/
   モード遷移)を診断に含めるべき。ECO-058 教訓(read-across 漏れ)の規模軸への拡張=N=3
   (064 起動→062 検索→113 選択)で**昇格候補**。
2. **「違和感の言語化」は工程診断の最短経路**: 報告者の「チェックが付くだけなのに件数に影響される
   のは違和感」という計算量直観が、そのまま診断仮説(選択コスト=母集合比例の構造検出)と
   probe 設計(母集合列挙の呼び出し回数)に直結した。症状の再現手順だけでなく**期待計算量との乖離**を
   起票に書く様式は再利用価値がある。
3. **旧 perf 是正の残余は「直った」記憶でマスクされる**: e39d68a(2026-06-29)は「選択クリックの
   応答性是正」を掲げて Items 全再構築を止めたが、O(母集合) 走査×3 は残っていた。「その症状は
   以前直した」という履歴が再診断を鈍らせる — 性能是正の記録には**残存する計算量**(何が O(N) の
   ままか)を明記する(本 ECO は §7.4 所見 3/4 で残余と検出限界を宣言=同じ轍を踏まない形)。
