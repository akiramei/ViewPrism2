...
### GF-TAGCTRL-05(追随・起票済・実装未着手): golden 承認版がモック(CAD)の簡略サブセット
...
**GF-05 は本セッションでは実装しない**(maintainer 指示: 工程記録優先)。別セッション/別 ECO で実装を mock へ寄せる。D1(幅 820)は headless プローブの再計測が必要(D6 フッターも連動)。

**残ゲート**: golden 最終承認(`50-as-built.yaml` golden_approvals へ CP-UI-G11 approved 記録)は maintainer の working tree 一括コミット時に確定 → ECO-022 クローズ。
