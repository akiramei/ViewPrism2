# Change Order — ECO-084(applied): 階層ビューの表示モード切替「すべて/未分類」(累積フィルタ+最深配置)
...
- **gate②(golden)**: ✅ **合格(2026-07-14 maintainer 実機・GF-084-01 是正を経て)** — 詳細 §10
...
**gate② 承認(maintainer 実機)**: ビュー軸でセグメント出現(FS 軸非表示)・未分類=最深配置
(ルート=未分類発見器)・チップ件数追随・分類進捗の動的減算+専用空状態・ビュー毎記憶の再起動復元・
狭幅収納(セグメント込み)・en 切替。第 1 回 golden の GF-084-01(§8)を同日是正し再 golden 合格。

**クローズ 3 点セット**: CP-DISPMODE-084 へ golden 承認+GF-084-01 潜伏実績を明記(再発防止)/
register `applied`+golden approved / 本節。**M4 同期**: spec §2.6 ツールバー行へ表示モード追記・
E-UI-AXIS-NAV-040(REQ-094+表示モード)・E-SETTINGS-013(viewDisplayModes)の name/refs 追補。
CAD 正典= ViewPrismUI(bf6d4cf/abd6cc2/4838972)。

**教訓**:

1. **共有部品は「同型に見える」だけで選ばない — 内容の可変軸(文字量・ロケール)で選ぶ**(GF-084-01)。
   グリッド/リスト切替と「同型のセグメント」という CAD 記述をアイコン用固定幅部品の流用と解釈し、
   en で破綻した。部品には適用条件(アイコン=固定幅 segBtn / テキスト=内容幅 segBtnText)があり、
   視覚 probe は日英両ロケールで余白を実測する。read-across= ECO-079(golden 全 ja 実施の谷間)の
   視覚版 — **i18n は文言だけでなくレイアウトの検査次元**。BomDD 昇格候補(R7 並置の検査次元に
   「可変内容の収まり(ロケール横断)」を明記する一般形)。
2. **新機能でも R5 の赤は撮れる — 導線不在をリフレクションで実行時不合格にする**(ECO-056 様式の
   再適用が有効に機能した)。
3. **ハーネスの遅延初期化×並列実行は「テスト集合の増減」で発火配置が変わる**(ECO-082/083 系)。
   Headless セッションの初期化はどのテストよりも先に同期完了させる(AssemblyFixture)。
   ModuleInitializer 内の Dispatch 同期待ちはローダー相互待ちでデッドロック(実測)— この間
   ECO-081 HangDump/ECO-083 FailFast の防御層が設計どおり多段で機能した(監視の実運用実績)。
