...
- **R8 セルフレビュー(出荷前レビュー)**。src に触れる fix は、golden 提示(golden n/a の
  ECO では accept 依頼)の**前**に、AI 自身が fix diff への**独立コードレビュー**を実施する。
  独立性の担保= fix を書いたコンテキストと別の fresh context(/code-review 相当の subagent。
  裁量で Codex 二次意見を追加)。所見は全列挙して処置を分類する:
  スコープ内欠陥=本 ECO 内でプローブ先行是正(R5 へ戻る)/スコープ外= R3(分離起票 or
  51-cheat-log)の二択。**未処置のスコープ内所見 0 になるまで golden に出さない**。
  レビュー結果(所見全数+処置分類)は ECO 実施記録へ記載する(maintainer は「AI が何を
  見たか」を前提にレビューでき、重複が減る)。doc-only・機械的 1 行変更(ECO-111 型)は
  「R8 対象外」を宣言して省略可(R7 の対象外宣言と同型)。目的は人間レビューの代替ではなく
  **所見の先取りによる往復削減**。
  背景: 2026-07-17 の maintainer レビュー 2 セッション(ECO-099/100 の未 push 12 コミット・
  ECO-103 fix `7370b7d`)が計 6 所見 → ECO-101/102/104/105 の 4 ECO を生んだ。いずれも
  潜伏 0〜1 日= diff 精読で先取り可能な種類で、accept 後のレビュー(ECO-103)は手戻りが
  最大化した(別 ECO 起票+golden やり直し)。golden 前のセルフレビューが最も上流で安い。
...
3. `/eco-fix <ECO>`: **プローブ先行(R5)** → 最小是正(真因構造を消す案を優先: ECO-038 では
   ...
   **セルフレビュー(R8)= fix diff の独立レビューで未処置スコープ内所見 0 を確認** →
