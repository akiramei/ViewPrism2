# Change Order — ECO-049(applied): EXIF Orientation の表示系適用 — V1 裁定済み沈黙次元(defer)の解除(スマホ撮影画像の横倒し表示)
...
- **maintainer 実機確認(基準 1〜4 全 OK)**: ①orientation_fixture_06.jpg(EXIF=6)がサムネ・ビューアとも正立表示
  ②解像度=実効寸法 834×1194 ③EXIF なし既存画像の表示回帰なし ④見開き/スクロール/タグ制御モード正常。
- **再発防止(CP 観点明記)**: CP-THUMB-007(EXIF 正立生成・実効寸法・キャッシュ世代移行・TopLeft 回帰=
  「横倒しが仕様だった」潜伏実績つき)・CP-UI-G8(EXIF 付き実画像の正立表示+読込経路差し替え時の
  全モード裏面検査・フィクスチャに EXIF 実画像を追加)。
- **M4 同期**: M-THUMB-008(ExifOrientationTransform/OrientedImageLoader 帰属・cache_key -v2・実効寸法・
  pHash 非適用 invariant)・M-UI-018(oriented_load 契約)。spec/E-BOM/K-BOM/41 は fix 時に同期済み。
  35-dsbom 不要(surface 新設なし)・62 不要(DB 不変 — キャッシュはファイル)。
- **教訓(一般形・昇格候補)**: **沈黙次元の裁定は「コード無し」と「挙動無し」を区別して記録する** —
  V1 裁定「原典も未適用」は原典コードの事実としては正だったが、原典の実行基盤(Electron/Chromium)が
  `image-orientation: from-image` 既定で暗黙に機能を提供しており、**ユーザー可視挙動としては適用されていた**。
  リバース工程の等価性判定は「明示コードの有無」でなく「ユーザー可視挙動」を基準に取り、実行基盤の
  暗黙機能(ブラウザ既定・OS シェル挙動・ランタイム既定)を沈黙次元の掃討対象に含める。
  read-across: ECO-041(CAD 定義済み機能の実装欠落=台帳間の転記漏れ)と対をなす「基盤暗黙機能の抽出漏れ」様式。
  副教訓: **パスキーのみのファイルキャッシュは生成規則の世代を持たないと規則変更で恒久 stale** —
  pHash P-09(adapter 世代)のファイルキャッシュ版。キャッシュ新設時は世代識別子を最初から設計に含める。
