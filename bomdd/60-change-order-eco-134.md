...
- status: applied(2026-07-22 クローズ・golden n/a=実機不要裁定)
...
## §8 クローズ(2026-07-22・golden n/a=実機不要裁定)

- **クローズ根拠**: 是正は挙動 bit 一致(候補選択=同ハッシュの id 序数最小を維持・視覚変更なし)。
  maintainer が「実機 golden 不要でクローズ」と裁定。よって gate② は n/a、機械受入が納品条件。
- **機械受入**: build 0 / Tests 924/924(スケールプローブ 40000→200 反転+挙動同値)/
  Oracle 109 pass+4 skip+0 fail(凍結オラクル無接触=R6)/ validate 0-0。
- **再発防止**: [33-control-plan.yaml](33-control-plan.yaml) CP-SCAN-004 に計算量 test_vector を fix 時に既設
  (候補照合は O(missing+new)・missing 行は新規件数 N に依らず M 回だけ走査=CpScanCandidateIndexTests で pin)。
- **M4 同期**: 不要(判定器内部の性能是正=挙動 bit 一致・surface/仕様値変更なし・as-built 乖離なし)。

### 教訓

**性能是正でも「挙動 bit 一致」を機械で裏取りすれば golden(実機)は不要にできる。** 本件は O(M×N)→O(M+N)
のアルゴリズム置換で、①スケールプローブ(走査回数 40000→200=二乗の消滅を実測)と ②挙動同値プローブ
(id 序数最小の選択が不変)の 2 軸で機械受入を閉じ、独立レビュー(R8)が旧 `Where→OrderBy→First` と
新・写像 lookup の bit 一致を裏取りした。視覚も観測挙動も変わらない是正は、実機 golden の代わりに
「不変条件を数える決定論プローブ」で受入できる(固定時間閾値を置かない方針=[[dotnet-test-execution-discipline]]
や ECO-058 の実測探索と整合)。read-across: 「見える不整合の多くは単一真実+キャッシュ/インデックス鮮度」
([[eco128-image-state-model-overhaul]] 教訓⑤)と同型で、候補照合の二乗は「毎回リストを線形走査」という
インデックス不在の一形態。
