...
### golden(2026-07-04・maintainer 実機)— **初回合格・第2段クローズ**

- 4 観点(トグル/追加チップ/作業タブ受け渡し/入口出し分け)全て従前どおり。**所見 0 件**
  (第1段は 2 件 → 本段 0 = 通知トポロジーの事前設計の効果が golden でも確認された)。

### 残ゲート(第2段時点の記載 — クローズ済み)
...
## 14. 第4段 裁定(2026-07-04): 切り出さない — §5 縮小条項の初適用

### 14.1 実物点検(定量)

- 対象クラスタ(軸ナビ+ソート/表示列)の実体は**フィーチャ状態でなくブラウズのデータフロー座標**:
  ResolveFs/ViewMatched/SortFiles/BuildListColumns は Recompute パイプラインの構成要素で、
  **ホスト中核状態 8 種**(_entries・_allViews・_viewConditions・_viewId・_viewRoot・_tagById・
  _tagFilter・_listColumnDefs)を読む。自前状態は 6(_axis/_fsPath/_viewPath/_currentChildren/
  _sortColKey/_sortColDir)に対し読み取り依存が上回る。
- コマンド 11 種(軸/パンくず/チップ/ソート/列)はメニュー開閉(ホスト UI 状態)とも結合。
  消費者: XAML 29 箇所・tests 多数(SelectedColumns/AvailableColumns/Chips/cluster 系)。

### 14.2 裁定

**切り出し中止**。前 3 段の境界基準(popup 型= UI 契約ごと/ストア型= 実体のみ)のどちらにも該当しない
第 3 の型= **データフロー座標型は切らない**。切る場合の実体は所有の移転でなく配線の複製
(getter 8+殻 11+コレクション往復)で、unit 粒度規準(独立に再製造・交換できる最小単位)に不適合。
挙動保存リスク(Refresh 中枢の通知トポロジー)が利得を上回る。§5「移送表で分類できないメンバ=
無理に切らず記録して段を縮小」の初適用。

### 14.3 系列目標の修正(第5段の再定義 — 実測に基づく)

「ハブの完全解体」→「**ハブの縮退+境界の明示**」へ:
- 第5段= M-UI-016 の lineage 確定: `split_by: [M-UI-TRASH-032, M-UI-WORK-033, M-UI-ORGANIZE-034]` を宣言し
  superseded へ。残余(ブラウズ+軸ナビ+ソート/列+モード統制+タグ付与パネル)は後継 unit
  **M-UI-IMAGETAB-035** として新 ID で宣言し、ebom_refs に **E-UI-BROWSE-022 / E-UI-AXIS-NAV-040 /
  E-UI-MODE-041 / E-UI-TAGASSIGN-029 を全数張る= 多:1 写像の明示化**。
- これで scale-01 の系統誤差の是正目標は達成される: under の原因は「宣言(1 品目)と実装(9 責務)の
  乖離」— 解体 3+複合宣言 1 で**宣言=実装**になり、影響分析はハブ残余を正しく予測に入れられる。
  ハブ台帳の M-UI-016 行は「縮退済み・残余は宣言済み複合(M-UI-IMAGETAB-035)」へ更新。

### 14.4 scale-02 所見(E 層へのフィードバック)

切り出し可能性の境界は「**フィーチャ状態(popup/ストア型)は切れる/データフロー座標は切れない**」。
E-BOM が品目として分ける BROWSE と AXIS-NAV は、M 層では単一データフローが自然= **品目→unit 1:1 が
理想という暗黙前提の反例**(多:1 を宣言で正直に張るのが正 — 61 §1.2 実物点検が「複合 unit の全品目」を
点検対象にすれば影響分析精度は保てる)。

---

## 15. 第5段+系列総括(2026-07-04 完了)

### 第5段(lineage 確定 — M 層 lineage の初適用)

- M-UI-016: `status: superseded`+`split_by: [TRASH-032, WORK-033, ORGANIZE-034, IMAGETAB-035]`+
  `superseded_by: IMAGETAB-035`。artifact.path は後継へ移譲(superseded unit はファイルを所有しない=
  遡及採点の帰属も後継へ)。ebom_refs 空化(全品目再帰属)。
- **M-UI-IMAGETAB-035** 新設: 残余の複合 unit — ebom_refs に BROWSE-022/AXIS-NAV-040/MODE-041/
  TAGASSIGN-029 の 4 品目を全数宣言(**多:1 写像の明示化**)。FMEA-014 再帰属。
- M-UI-013: 画像タブ 3 品目を 035 へ再帰属し catch-all 縮小(残= SHELL/NODEGRAPH/TAGS/DESIGN)。
- 検証: workspace lint error/warn 0・遡及採点ツールの帰属が後継へ正しく移ることを確認 —
  **旧ハブ under(10/16 ECO)は「宣言=実装」化により予測内へ変わる**(scale-01 の系統誤差是正の機構が
  ツール上で確認できた)。ハブ台帳は 035 を後継として既定点検を継続(複合 unit につき)。

### 系列総括(scale-02)

| 段 | 方式 | 設計凍結 under | golden 所見 | 帰結 |
|---|---|---|---|---|
| 1 ゴミ箱 | popup 型×工場(列なし移送表) | 4 | 2(通知不足) | M-UI-TRASH-032 |
| 2 作業 | ストア型×設計者(列あり) | **0** | **0** | M-UI-WORK-033 |
| 3 整理 | 転送殻×工場(列あり) | 1(R3 通知穴) | 1 →**A/B で既存バグと判明** | M-UI-ORGANIZE-034+ECO-037 発見 |
| 4 軸/ソート | — | — | — | **切らない裁定**(データフロー座標型) |
| 5 lineage | — | — | — | superseded+複合宣言=宣言=実装 |

- god-VM 1,765 行 → 3 子 VM(計 ~600 行)切り出し+残余は 4 品目複合として正直に宣言。
- **主発見**: ①切り出し境界の三型(popup= UI 契約ごと/ストア= 実体のみ/データフロー座標= 切らない)
  ②通知トポロジー 3 則(不足も過剰も所見・旧版と回数/位置同一が正・golden が通知面の唯一の検査器)
  ③挙動保存検証は既存バグの検出器(ECO-037= 3 週間潜伏の実害を発見)④教訓の学習効果が定量で出た
  (under 4→0・golden 所見 2→0)⑤品目→unit 1:1 理想の反例(多:1 の明示宣言が正)。
- 工程事故: 工場の入れ子委譲ループ+並行製造(第3段・51 裁定)— 隔離規律 3 点を還元候補化。
