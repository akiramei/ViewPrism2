# ECO-120(applied): 検査器の欠陥 — validate_bom が台帳 15 本中 4 本しか読まず、33-control-plan の構文破壊でも 0/0 を返す
...
- クローズ: 2026-07-20(golden n/a=機械証拠検収・§9)
...
## §9 クローズ(2026-07-20・golden n/a=機械証拠検収)

### 9.1 検収(maintainer の accept 指示)

機械証拠 3 点: ①破壊注入の赤/緑全系列(§7.1= 33 構文破壊で E20 exit 1・未読 31-kbom の
重複キーで E13 exit 1・復旧で 0/0) ②selftest の E20 陽性対照 OK+selftest-lifecycle OK
③受入 4 点不変(build 0/0・Tests 844/844・Oracle 109+2skip)。

### 9.2 再発防止

再発防止は**検査そのもの**(E20 構文床+E13 全数化)と **selftest の E20 陽性対照**
(pre-commit が `--selftest` を毎コミット実行するため、検出能力の退行が即日露見する)。
golden CP を持たない検査器 ECO につき CP 刻印は非該当(ECO-105/111 前例)。M4 同期も不要。

### 9.3 教訓

1. **検査器の検査範囲は「読んでいるファイルの一覧」でしか保証されない。**
   E13(ECO-053)は「全 YAML 台帳に適用」と docstring に書かれていたが、実際には
   load される 4 本にしか効いていなかった — **検査の宣言と検査の実装は別物**で、
   宣言だけを読むと全数掃射済みに見える。ECO-107 教訓 2(粗スキャン結論は全数調査で再検証)の
   検査器版であり、ECO-119 教訓 1(散文は全体が真である前提で読まれる)と同じ機序が
   docstring にも働く。是正では docstring 側も R8 が矛盾を検出して同期した(実装が正)。
2. **fail-open な外部検査に床を依存しない。** 意味検査(bomdd-lint)は隣接リポの CLI に
   依存し pre-commit では fail-open= CLI 不在の環境ではコミットが素通しする。
   正本リポの fail-closed ゲート(validate_bom)が**最低限の床(構文健全性)**を自前で
   持つことで、外部検査は「より深い検査の追加層」に位置づけ直された。
   ECO-081(HangDump の不作動に備えた基盤上限)と同じ**多層防御の設計原則**=
   最終安全弁は自リポ・fail-closed 側に置く。
3. **陽性対照は検出能力の退行ガードとして常設する。** E20 を足しただけでは
   「E20 自体が壊れて素通しに戻る」退行を検出できない。selftest へ合成の壊れた YAML を
   置いたことで、検出能力そのものが毎コミット検証される(ECO-053 が E13 で確立し、
   ECO-078 教訓「陽性対照で空振り防止」・ECO-107 lint の合成違反サンプルに続く様式= N=4 級。
   BomDD 昇格候補: 「検査を追加する変更は、その検査の陽性対照を同梱する」を受入条件へ)。
