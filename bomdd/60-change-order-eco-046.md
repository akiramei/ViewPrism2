# Change Order — ECO-046(applied): タグ削除×未保存ビュー階層編集の谷間 — 保存が FK 違反の未処理例外(「予期しないエラー」)
...
### 実機確認(maintainer・基準 1-4 全合格)

①再現手順の再走= 未保存配置タグの削除が確認ダイアログの前に拒否・理由表示・配置無傷
②編集キャンセル後は削除成功 ③保存後は TagInUse(ECO-045 の DB ガードへの委譲)
④配置→保存の回帰正常。**発端の症状(保存で「予期しないエラー」)は操作到達不能になった**
(UI ガードで削除自体が拒否+万一の別経路も Core 防御が Result 化)。

### 再発防止(CP 明記)

- **CP-UI-G6**: 未保存編集×削除の確認前拒否の観点(潜伏実績= DB 参照ガードだけでは
  未コミット編集を関知できず ECO-045 golden 中に顕在化)を明記。
- **CP-VIEW-012**: 参照切れ保存拒否ベクタ(NotFound・無傷・S-39 と二重化)を追加。
- M4: 20-spec §2.2(fix 内)・M-VIEWSVC-012 interface_contract.views(参照切れ拒否)。

### 教訓

**不変条件は「正の状態(DB)」と「作業中の状態(メモリ内編集)」の両面で守って初めて閉じる。**
DB 参照ガード(ECO-045)は導入した瞬間から「参照がまだ DB に無い編集面」という補集合を持ち、
そこが未処理例外の露頭になった。是正の型= ①操作入口(UI 層)で作業中状態を含めて判定し、
②永続化入口(Core)は参照切れを Result で拒否する防御層を敷く(単層で済まさない)。
read-across: ECO-045 教訓(DB ガード×未保存編集の谷間)の実証+完結編。ECO-037(view 層でしか
現れない欠陥)・ECO-041(未配線)と同じく「検査が届かない面」系 — 本件は golden 中の
maintainer の探索的操作(基準外の手順)が谷間を突いた= **golden は台本の消化でなく探索を含むことに
価値がある**(基準は最低線・所見は台本外から来る)。一般形(方法論昇格候補): 「ガード導入時は
その参照面の補集合(未コミット状態・キャッシュ・転送中)を列挙し、各面の防御を設計する」。
