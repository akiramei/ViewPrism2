...
### 実行確認内容(maintainer・2026-07-13)

`dotnet test tests/ViewPrism2.Tests` = **664/664 全緑**(全 View lint=src 配下全 axaml+陽性対照込み)を手元で実行確認。UI 変更なしのため実機視覚確認は対象外。

### 再発防止の所在

- **機械ゲート**: `CpI18n010XamlLintTests.全Viewに直書き日本語文言が残っていない`(src 配下全 axaml・Title 含む 7 属性・列挙 ≥20 ガード)+陽性対照。CP-I18N-010(automated)が正本台帳。
- **参照導線**: /eco-fix スキル手順 2 の横断規約適合確認(K-BOM を正本として指す)。

### 教訓

**横断関心事(i18n・a11y・テーマ等、モックが構造的に沈黙する決定事項)は「①宣言(K-BOM/REQ)+②作業時の参照導線(スキル手順)+③機械ゲート(lint)」の 3 層で運用する**。宣言だけでは製造時に読まれない(ECO-079=K-AVALONIA が明文でも 2 面すり抜けた実績)、ゲートだけでは規約の意図が伝わらない。②の置き場は「毎セッション読み込まれる文書(CLAUDE.md)」より「全作業が必ず通る手順書(スキル)」が強い — 手順として実行されるため。予防ゲートには赤プローブが存在しないので、**陽性対照が赤プローブの代替**になる(ECO-078「検査の暗黙前提は陽性対照として持つ」の予防ゲート版)。read-across: ECO-079 教訓①(層横断棚卸し)の運用形であり、R5(プローブ先行)・R7(セルフゴールデン)がスキルに織り込まれて機能している既存実績の第 3 例。

### 残課題(スコープ外・将来候補)

- VM 層 lint の全 VM 一般化(現在は画像/作業タブの live VM 4 ファイル限定。他 VM 群の直書き実態は未棚卸し)。
- 3 層運用の一般形の BomDD 昇格(方法論リポ側の変更=別オーダー)。
