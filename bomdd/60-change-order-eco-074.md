# ECO-074(applied / クローズ済み 2026-07-13): バックアップファイルの置き場所管理(B層パッケージの既定フォルダ規約)
...
- §9 の 5 項目すべて合格: 書き出し既定出力先=`<文書>\ViewPrism2\collections`+保存ダイアログ同起点/
  取り込みで picker 自動起動(起点=管理フォルダ)/キャンセル残留プレースホルダ/逸脱先を覚えない/ja・en。

### 10.2 再発防止(恒久化の所在)

- CP-UI-G13 characteristic へ置き場所観点を潜伏実績つきで追記。機械側は `CpPackage074Tests`
  (既定出力先・自動起動・プレースホルダ)。規約の正本= CAD「パッケージの置き場所」節
  (ViewPrismUI e26e29d)+仕様 §2.14.7。

### 10.3 教訓

- **重要操作の「置き場所」は要求の一次元として明示的に設計する**: バックアップという操作の性質
  (誤選択の重大さ・持ち出し・目視確認)から置き場所規約が導かれるのに、CAD/spec とも「出力先」と
  いう UI 部品の存在だけを規定し、既定フォルダと picker 起点は沈黙だった。しかも A層(SS-002)は
  同じ次元を既に解いており、**同一機能族の先行裁定(管理フォルダ+settings 永続)を後続設計の
  チェック項目として read-across する**べきだった。ECO-073 GF 系の「水平展開漏れ」の設計工程版=
  同族(視覚言語は面へ、設計裁定は機能族へ、それぞれ全列挙して展開する)。方法論昇格候補。
