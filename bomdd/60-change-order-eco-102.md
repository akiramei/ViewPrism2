# Change Order — ECO-102(クローズ・applied): 未保存編集中のタグ定義変更が中央ペインへ反映されない — dirty ガードによる行 Tag 参照の失効
...
dirty 編集中のタグ改名+色変更が中央ペインの該当行へ即追随・ツリー構造/別名/条件/HOME/dirty 状態は
不変・そのまま保存で階層が無傷に永続・数値範囲変更で行メタ即追随・配置モード中の改名で帯追随・
非 dirty 時の従来挙動(全再ロード)に回帰なし。

### 9.2 再発防止

E-UI-NODEGRAPH-025 へ「構造の保護と表示の鮮度の分離」invariant、CP-UI-G6 へ dirty 中タグ編集反映の
検査次元(潜伏実績= c103967 から 1 か月超・ECO-046 の削除面と対になる編集面)を刻印(fix 時)。
機械側は CpUiG6DirtyRebindTests(4 本)が改名/色/数値範囲の即追随・構造/dirty 不変・保存回帰・
placing 帯追随・非 dirty 従来経路を恒久 pin。

### 9.3 教訓と read-across

1. **保護ガードは「何を守るか」を分解してから置く** — dirty ガードの保護対象は未保存ツリーの
   「構造」だったが、実装は再ロード全体をスキップし「表示の鮮度」まで一括で犠牲にした。
   ガードを置くときは保護対象と巻き添えを分けて宣言する(是正= RebindTags で二つを分離)。
   read-across= ECO-046「ガードは DB と作業中状態の両面で閉じる」の裏面 — ガードが**閉じすぎる**
   欠陥もある。同じ dirty 谷間から削除面(ECO-046)と編集面(本件)の 2 欠陥が別々に出た=
   「状態×操作のマトリクスで谷間を掃射する」が一般形(BomDD 昇格候補)。
2. **参照オブジェクトの鮮度は生成時固定と相性が悪い** — EditNode が Tag オブジェクトを生成時に
   固定して以降の全表示を導出する構造は、グローバル定義の変更に対して失効が構造的に発生する。
   永続定義を参照表示する VM は「再束縛の経路」を最初から契約に含める(ECO-095 の
   「ロケール文字列の永続データ焼き込み」と同族= 生成時スナップショットが後変更を見失う様式)。

### 9.4 M4 同期・残課題

- M4= fix 時同期済み(E-UI-NODEGRAPH-025/CP-UI-G6)。spec/35-dsbom 不要(VM 内部+invariant 追補)。
- 残課題: なし。レビュー 4 所見(ECO-101 の 3 件+本件)はこれで全消化。
