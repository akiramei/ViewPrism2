...
### P3(中): UI-IR ロケーションが 2 系統に分裂 → ✅ **裁定済(2026-07-01・maintainer 裁定 A)**
...
- **裁定(2026-07-01・maintainer 選択 A)= ViewPrismUI 単一権威 + bomdd/ui 凍結**: 単一権威=`ViewPrismUI`(`資料/` モック + `docs/screens/` プロース CAD)。`bomdd/ui/`(JSON 抽出)は現行 E-BOM をブートストラップした歴史的成果物として**凍結**(保存・非保守)、乖離時は常に ViewPrismUI が正。正典フロー= mock → `docs/screens/<screen>.md` → ECO → E-BOM(ビューアー ECO-022 が既にこの経路=実態一致)。記録: `bomdd/ui/README.md` 冒頭に凍結宣言・`ViewPrismUI/docs/02_mock_fidelity_policy.md`「UI CAD の単一権威」節。BomDD 方法論の JSON 抽出ステップは本プロジェクトでプロース CAD+ECO に置換(project-level supersede)。
...
### 進捗(2026-07-01)
- **P1 = 完了**: `image_viewer.md`(+`image_viewer_tag_control.md`)新設。乖離 V1-V5/D1-D9 検出→ V2/V3/V4 裁定 + GF-TAGCTRL-05 追随実装 + CP-UI-G11 golden approved(ECO-022 クローズ)。
- **P2 = 完了**: 作業タブを ViewPrismUI へ統合(`資料/作業タブ/` + `docs/screens/work_tab.md`)。
- **P3 = 裁定済**: ViewPrismUI 単一権威 + bomdd/ui 凍結(上記 P3 節)。
- **P4 = 方針のみ(当面 原典準拠=意図的と記録)**。
