# Change Order — ECO-101(クローズ・applied): タグタブ操作系の入力堅牢性 — ポインター押下状態の残留・右クリック配置・無変更ホーム設定の dirty 化
...
### 9.1 実機 golden(gate②合格・maintainer)

配置モード中の右/中クリックで階層不変・現ホーム行への「ホームに設定」で保存非活性(他行へは従来どおり
排他移動+活性化)・クリック配置/行 D&D/パレットドラッグ/行クリック(展開・選択)の回帰なし・視覚不変。

### 9.2 再発防止

CP-UI-G6 へ「入力堅牢性」次元(右クリック非破壊・非押下移動の非発火・無変更操作のダーティ不変)を
潜伏実績つきで刻印(fix 時)+「headless セッションはポインタ押下状態をテスト間で共有=マウステストは
Down/Up を対にする」の計装注意。機械側は CpUiG6InputRobustnessTests(4 本+クリック経路 pin 2 本)が
恒久 pin — 入力イベント層(ボタン種別・キャプチャ喪失)を VM コマンド直接実行では届かない面として
headless 実マウスイベントで検査する様式を確立。

### 9.3 教訓と read-across

1. **VM コマンド直接実行の probe は入力イベント層を素通しする** — ECO-099/100 の probe 群
   (19 本)は全て緑のまま本欠陥 3 件が併存した。ジェスチャ・ボタン種別・キャプチャ喪失の契約は
   headless 実マウスイベント(MouseDown/Move/Up)でしか検査できない。read-across= ECO-041
   「未配線 UI は 1 回操作して結果を見る」の入力層版(操作 UI は golden/機械とも**実イベント**で
   1 回叩く)。
2. **持続観測へ変換して赤を固定する** — 「誤ドラッグが始まる」は transient で assert できないが、
   「既存のクリック配置モードが破壊される」は持続する副作用として決定論で赤にできた。
   probe 設計は「欠陥の瞬間」でなく「欠陥が残す状態」を掴む(GF-092-02 の headless 非再現の
   処し方の発展形)。
3. **是正前の欠陥はテスト計装自体も汚染する** — in-flight DoDragDropAsync とポインタ状態の
   テスト間共有により、是正前は 4 probe が同時に赤にならなかった(2 回の実行に分かれて全赤を実測)。
   赤の実測が不安定なときは「欠陥がハーネスへ漏れている」ことを疑う。

### 9.4 M4 同期・残課題

- M4= CP-UI-G6 のみ(fix 時同期済み)。E/M-BOM は契約不変のため対象外。spec/35-dsbom 不要。
- 残課題: なし(同時レビューの所見②= ECO-102 で別途クローズ)。
