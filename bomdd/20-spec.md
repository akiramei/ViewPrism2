# 仕様書 — ViewPrism2 (BOM v2.0)

> 製造パッケージに含まれる(製造装置が読む)。REQ への双方向トレースを保つ。
> 原典: view-prism(TypeScript/Electron)のリバース仕様。原典ソースは工場非開示のため、本書が機能記述の唯一の典拠。
> 版: v2.0(loop-v2-viewer 改訂 — §2.9 ビューア拡張・REQ-054〜060 追加・INV-009 woven 格上げ・GF-01〜05 是正。
> §2.0〜2.8 の v1.3 確定分は loop-v1-core の凍結オラクル(tag loop-v1-r1)で保護され、意味論の変更はない)
...
| タグ制御モード(タグでページ配置を自動制御) | Loop V3(2026-06-13 裁定 — 原典でもマッピング設定 UI・永続化が未実装の未完成機能) |
| Undo/Redo | Loop V3 |
