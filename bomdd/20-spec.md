...
- ビューア (REQ-044): 単一画像をウィンドウフィット表示(縮小のみ、拡大なし)。→/PageDown=次、←/PageUp=前、
  ...
  ウィンドウ方式への適応。v1.3/CR-7)。並びは呼び出し元一覧の整列結果。端で停止(ループ・例外なし)。
  表示中はタイトル等に「現在位置/総数」を表示
...
非モーダル通知(ステータスバー)で利用者へ示す。検証は CP-ROBUST-001(NFR-002)。
...
- G-4 ビューア: 〔グリッドの画像をダブルクリック〕で起動。画像がウィンドウに収まり(縮小のみ)、端で停止。
