# ECO-064 (applied) — 大規模コレクションで初期起動をブロックせず読み込み状態を表示する
...
5. ~~gate② golden: 大規模profileでshell-first、loading状態、応答性、ready/error遷移、既存復元回帰を実機確認する。~~ → maintainer実機approved(2026-07-11、§8)。
...
- maintainer実機でcold start時にshellが先に表示され、loading中もwindow移動/resize、タブ往復、設定が応答することを確認。
- 左catalogの「コレクションを読み込み中…」から行+件数、中央contentの「画像を読み込み中…」から画像へ二段で遷移することを確認。
- loading途中に「コレクションを選択してください」「画像がありません」を誤表示しないことを確認。
- content loading中に別collectionを選び、旧collectionの遅延完了で現在表示が戻らないことを確認。
- ready後のLastCollection復元、FS/view軸、home初期遷移、grid/list、scan中表示に回帰がないことを確認。
- error/retryの故意障害は実機データを破壊せず、CP-STARTUP-028のfail-once機械probeで承認。

### 8.2 再発防止

- `CP-STARTUP-028`: scheduling境界(Task.Yield/Task.Run)だけでなくcardinality境界(全件API 0、catalog集約、選択folder exact)、状態遷移、error/retry、遅延結果破棄を決定論固定。
- `CP-UI-G1`: 大規模cold startのshell応答、二段loading、空状態排他、切替裏面、既存復元を潜伏実績付きgolden観点化。
- `P-01`: maintainer実機の定性観測を記録。固定ms閾値は新設せず、件数・壁時計未記録を明示。
- `50-as-built.yaml`: `golden_2026_07_11_eco064`へ承認記録。

### 8.3 教訓

`Task`を返すAPIや`ConfigureAwait(false)`の存在だけではUI非ブロッキングを証明せず、仮想化もDB materialize量を制限しない。大規模データ経路は、**どのthreadで実行するか(scheduling boundary)**と**何件を処理対象にするか(cardinality boundary)**を別々の契約・検査にする。これはECO-059の明示`Task.Run`とECO-062のfilter先行を起動経路へread-acrossした一般形であり、片方だけの最適化では別の待ち時間を温存する。

### 8.4 残差

ECO-064に帰属する残件なし。固定起動ms閾値、全画面共通loading component(VP-UI-005の残り)、startup paging/streamingは本ECOで新設しない。必要なら別ECOで区分計測・CAD裁定から扱う。
