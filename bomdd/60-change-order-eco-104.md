# Change Order — ECO-104(applied): 保存バー表示状態の残欠陥 — トーストの dirty/ビュー切替への残留+保存失敗文言の言語非追随(ECO-103 レビュー所見 P2×2)
...
## 8. クローズ(2026-07-17・golden approved)

**maintainer 実機承認(改訂版基準 3 点)**: ①保存→即編集(1.8s 内)でトーストが即消え未保存バーのみに
なる ②トースト表示中の別ビュー選択で新ビューへ持ち越さない ③dirty 中の言語 ja↔en 切替でバー文言が
即追随する — 全点合格。

**再発防止の場所**: CP-UI-G6(交差ケース= トースト×dirty/ビュー切替・失敗理由の権威値保持+表示時
解決・golden 基準の到達可能性教訓)/ CpUiG6SaveBarTests 2 本(タイマ凍結による状態遷移側解除責務の
分離検査・ja↔en 往復=恒久ガード)。

**教訓**:

1. **一時表示の解除経路はタイマだけにしない — 表示を無効化する状態遷移の全列挙が契約**: トーストは
   「クリーン状態の一時確認」であり、その前提(クリーン・当該ビュー)を壊す遷移(dirty 化・再読込・
   ビュー切替)が起きたら即時解除する。タイマは「前提が保たれたままの寿命」でしかない。既存教訓との
   関係= ECO-046/102 系「dirty 状態の谷間」の表示状態版(状態×操作マトリクスの掃射候補=ECO-101 の
   BomDD 昇格候補と同族)。
2. **表示文言の権威値は再導出可能な形で保持する(ECO-095 教訓の VM 一時状態への外延)**: Resolve 済み
   文字列の保持はロケール切替に追随できない。ErrorCode/キーを保持し表示時に現在ロケールで解決する。
   同族欠陥がパレット面でも実機観測され ECO-106 として分離起票済み — 静的 i18n lint の死角
   (解決タイミング次元)として lint 拡充候補へ記帳(51-cheat-log・これで検査 3 次元)。
3. **golden 基準は書いた時点で到達可能性を検証する(ECO-045 教訓の再犯)**: 当初基準③は自リポの
   ガード(ECO-046)が閉じた経路を要求していた。基準を書く際は「その操作は今のガード構成で実際に
   実行できるか」を先に確かめ、到達不能な検査面は機械プローブへ振替する。再犯の構造= 教訓は CP に
   刻印済みでも、基準を書く手がガードの現況を照会しなければ再発する(教訓 1 の GF-103-01 と同型=
   「規約は存在するだけでは伝搬しない」)。

**スコープ外の後続**: ECO-106(パレット常駐メッセージ・staged)・ECO-105(撮影ハーネス・staged)。
