...
- **gate①(裁定)— 必要**:
  1. **初回スキャンの新規の扱い**: (i) 設計まとめ字義どおり全 pending / (ii) 初回のみ normal
     (現行 isInitialScan 分岐の流用=フォルダ登録は登録行為自体を裁定とみなす)/
     (iii) ECO-130 の「適用」を裁定とみなし新規は normal。26 万件登録の裁定爆発と
     「再出現しただけで無条件 normal にしない」原則の間の線引き。
  2. **pending の可視化**(INV-010 改訂): 既定一覧に出すか(バッジ付き)・裁定画面のみか。
     内容変更→pending で「画像がビューから消える」を許容するか。
  3. **固定オラクル旧行の処置**(R6 衝突・ECO-128 と一括裁定推奨)。
  4. **CAD mock の作成**(ViewPrismUI 側 gate: 裁定 UI・pending 表示)。
