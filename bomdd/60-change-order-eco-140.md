...
## §10 GF-140-02 — 3 択と ✕ 意味論の面上明示(案①裁定・2026-07-24)

**裁定(maintainer)= 案①**: GF-140-01 の本体表示回復に加え、IR-2 callout 補足へ淡色 1 文
「自動裁定せずに下の一覧から個別に裁定することも、閉じてあとで裁定し直すこともできます
(閉じても画像は変わりません)。」を追記(✕ 意味論= 裁定済みのみ確定・破棄なしは spec §2.11.8 の
既定・面への転写)。案②(文言なし)・案③(「個別に確認」ボタン=構造改版)は不採用。

**実施**: CAD= ViewPrismUI integrity_review を **Reopened**(理由+対象 ECO 記帳)→ mock IR-2+
...
## §12 クローズ(2026-07-24・gate② 実機 golden 合格)

### 実機確認内容(maintainer)

移動→自動裁定(relink)で新パスが元 image_id/タグのまま通常表示へ+元の missing(リンク切れ)解消/
再出現一致→そのまま受け入れ/別内容の再出現→自動対象にならず個別へ(案a= baseline 保全の封止確認)/
見つからない→条件検索(4 項目)→候補カードへ再リンク・ゴミ箱へ移動(復元可)/個別 4 操作+曖昧ケースの
手動回避/入口統合(旧 2 行不在・en 截断是正後の「Images to review…」)/**IR-7 動的挙動= 確認完了時の
一括反映(途中加算なし)+確認中の自動グループ見出し表示で裁定**(CAD 両義の沈黙次元を実機 golden で確定・
as-built 正典化)/回帰= 未裁定バッジ・ゴミ箱・既存確認面・ja/en 切替・空状態/データ後始末= 残存
missing/孤立タグ 0(GF-139-01 教訓の継承)。golden 1 巡目所見 3 件(GF-140-01 本体不可視・GF-140-02
3 択明示・GF-140-03 en 截断)はいずれも是正確認済み。GF-140-02 の CAD Reopened 分は本確認が再 golden を
兼ね再凍結。

### 再発防止

CP-INTEGRITY-036 へ golden 観点+GF-140-01(既定選択の全体フォールバック+無選択 probe)+GF-140-02
(一括 callout の 3 択/✕ 安全明示)+GF-140-03(メニュー行の ja/en 文字数予算 lint)を潜伏実績つきで刻印。
旧修復系 CP(023/021/G10)は superseded 注記済み・応答性封止は ECO-075 移植プローブが継承。

### 教訓

- **統合 ECO の受入は「旧面の意味論ベクタ台帳」を先に作る**: 旧 4 テスト(1,500 行超)の撤去分を R8 が
  全件突合して初めて「黙って消えたベクタ」(ECO-075 応答性)を検出できた。面の置換では“新面が緑”と
  “旧面の保証が生き残った”は別命題(ECO-087「代替導線の撤去はマスクされた欠陥を顕在化」の受入版)。
- **工場の停止条項は仕様欠陥の検出器として機能する**: 「migration 必要なら停止して報告」の一文が、
  設計者・レビューが見落とした REQ-103 の比較空虚化(スキャン上書き)を製造前に停止させた。
  委譲ブリーフの制約は“工場を縛る”だけでなく“正本の矛盾を早期に顕在化させる”(factory-delegate 昇格候補)。
- **一時状態専用の選択/表示ロジックは「対象空」の枝で必ず全体フォールバックへ倒す**(GF-140-01。
  ECO-131 世代ガード・ECO-104 交差ケースに続く“一時状態×既定状態”の谷間の 3 例目)。
- **bdr-01 2 系列目= 前提疑義の初観測**: 境界予測は「判定」への接触だけを見ており「判定結果の適用が
  裁定基準を破壊する」経路を落とした。1 系列目(3/3 想定範囲内)で未検証だった review_trigger の
  弁別力に反証側データ点がついた= 実験としては最良の収穫(§42 測定は BomDD 側へ記帳)。

### 残(後続)

- INT-004 確定処理= 本クローズと同便で ViewPrismUI 側実施(旧面 superseded 化+scan_summary 出口文言追随+
  integrity_review 再凍結記録)。
- cheat-log 記帳分= ①SQLite in-flight query interrupt(DB 基盤の分離候補)②既存メニュー行の en 截断+
  予算 lint 水平展開。
- bdr-01 §42 測定 3 点(2 系列目)= BomDD リポへ記帳。
