# ECO-067 (applied) — pHash 距離0の非同一画像が100%表示される重複品質契約の見直し
...
## §12 gate②合格・クローズ(2026-07-11)

maintainerが`/eco-accept ECO-067`で実機goldenを承認した。画像/作業タブで、表情差候補が70%検索へ
見た目に応じた複数の連続値で残り100%にならないこと、80%/95%へ上げると段階的に絞れ70%へ戻せること、
正規化画素exactだけ100%となることを確認した。再圧縮/resize/回転鏡像/trimの候補性、条件検索、停止、候補追加、
merge/Undoにも回帰なし。内部`類似/部分重複`の自動削除・自動投入禁止を維持する。商用画像・スクリーンショットは
最後までrepoへ収載していない。

再発防止はCP-DUPQUALITY-030の合成fixture/pHash距離0/v3 cache再検証、CP-UI-G9/G1の単一連続一致度・
threshold往復・exact100観点、50-as-builtの承認証拠へ固定した。M4はGF-067-04 fix時にREQ-090、仕様§2.10.4、
E/M-BOM、Design System BOM、CAD IMG-021まで同期済みで、accept時の追加乖離はない。

教訓: 内部分類を利用者向け数値へ固定帯で直結すると、分類境界が表示の不連続な崖になり、精度改善そのものが
検索体験を破壊し得る。安全分類と表示順位は同じ測定を共有しても出力契約を分離し、goldenでは代表値だけでなく
しきい値往復時の候補集合の連続性を確認する。これはECO-037の「非表示裏面も検査する」教訓の数値意味論版read-acrossである。

残課題: テンプレートマッチング/位相相関/局所SSIM/特徴点照合は、現fixtureまたは実機で不足が再観測された場合の
比較候補としてdeferする。現時点で新規依存を追加しない。
