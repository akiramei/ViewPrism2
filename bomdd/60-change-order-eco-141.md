...
- status: applied(2026-07-24 gate② 実機 golden 合格でクローズ)
...
## §8 クローズ(2026-07-24・gate② 実機 golden 合格)

### 実機確認内容(maintainer)

Avalonia 12.1.1 上での **①描画の見た目**(テキストのにじみ/字間・アイコンの縁・境界線の太さ。
重点=対照実験で差分の大きかったタグタブ〔保存バー/未保存表示/配置中〕・画像タブのツールバー/
ファイル操作・統合裁定面〔IR-2/IR-5/IR-6〕・スキャン結果確認)**②入力とバインディング**
(ダブルクリック起動 G-4・ロケール切替の即時反映 G-5= S-BOM `known_traps` が Avalonia 更新時の
要再確認として挙げる項目)**③仮想化**(ItemsRepeater 12.0.0 据え置きでの大量画像グリッドスクロール・
REQ-041)を確認し**合格**。

### 再発防止

`CP-UI-G1` へ 2 観点を潜伏実績つきで刻印:

1. **外部 UI ライブラリの版更新は「機械検査が全緑」でも実機 golden を省略しない**+
   **版更新 ECO の golden 判定手順**(①機械検査 ②CaptureHarness で旧版/新版の対照撮影+PNG 差分
   ③差分ゼロなら n/a 提案可・差分ありは実機 golden)。
2. **ハーネスの故障封じ込め契約**(内部フィールド pin →挙動契約 pin への置換・封じ込めの限定・
   撤去による診断解像度の後退)。

台帳の同期: procurement(版+記載漏れ 2 件の是正+Diagnostics 行除去)・M-HARNESS-015 `fail_closed`②・
S-BOM(`pinned_version` と版行 ×8)。

### 教訓

- **「機械検査が全緑」は「変化がない」ではない — 検査が見ている次元を確認する**。Gf 43 件は
  **レイアウトの数値**を測っており、レンダラ更新が動かす**描画そのもの**は測っていなかった。
  n/a の提案は「全緑」を「無変化」と読み替えた誤りで、**S-BOM が同じ危険を `reinspect_depth: G` として
  既に記帳していた**(ECO-002 実証)。台帳は読まれて初めて効く=**版更新 ECO では S-BOM の
  `drift_source`/`reinspect_depth` を起票時に引くことを手順化すべき**(BomDD 昇格候補)。
  read-across= ECO-134「性能是正は挙動 bit 一致で実機 golden 不要化」の**適用範囲の誤拡大**を防ぐ実例。
- **上流が直したと宣言しても、自分の依存の仕方に対して直ったかは実測で確かめる**。本件は
  変更ログ(「fix headless session hang when cleanup throws」)だけを根拠に撤去すると、
  **封じ込めの範囲を過大評価**するところだった(IL 実測で consumer loop の catch は不変=
  封じ込めは外部到達 2 経路のみと判明)。**陽性対照つきの再現ハーネスを資産として残しておくと、
  修正検証がそのまま回せる**(上流報告のために作った repro が 10 日後に検証装置として再利用できた)。
  ECO-081 教訓「陽性対照が手段選定を救った」の再演。
- **防御を外すときは、その防御が担っていた機能を分解して数える**。FailFast 監視は
  「#21770 の回避」と「原因例外の即時顕在化(診断装置)」の 2 機能を持ち、上流修正で消えたのは前者だけ。
  後者の喪失を契約へ明記して初めて、次の再発時に「なぜ証拠が薄いのか」を辿れる。
  ECO-083 教訓「fail-fast 監視は診断装置を兼ねる」の裏返し。
- **私的 API に依存する防御は、前提を機械 pin し、撤去時は前提の消滅を挙動で実証する**。
  本件は pin が緑(フィールド残存)のままだったため、**pin だけでは撤去可否を決められない**ことが露呈し、
  裁定が要求した挙動裏取りが実質的な判断根拠になった。置換後は**依存対象そのもの(挙動契約)を pin**
  しているため、この二段構えは不要になった。

### 残(後続)

- **cheat-log 記帳**(本 ECO 起因でない or 将来リスク): ①`CLAUDE.md`「Oracle skip は既知 2 件」が
  実測 4 件(ECO-128 以来の stale)②Headless の poison 経路で出る `UnobservedTaskException` は
  将来 `ThrowUnobservedTaskExceptions` を有効化するとプロセス毒になり得る。
- **ECO-082**: 保留静置を維持(§9 に再測観測を追記済み)。**再発時の一次証拠が HangDump 経由へ後退**した点に留意。
- **`CpUiG6SaveBarTests` の別失敗モード**(`ObjectDisposedException 'TestContext'`)は本 ECO で解決を
  約束せず(今回のフル run では未発火)。再発すれば別起票。
- **BomDD 昇格候補**: 上記教訓のうち①「機械検査が見ている次元の確認+版更新時の S-BOM 引き当て手順化」
  ②「防御撤去は機能を分解して数え、失われる機能を契約へ明記」は方法論レベル。
