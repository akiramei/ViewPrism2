...
- **案A(段階撤去・推奨)**: 12.1.1 へ一括更新(ItemsRepeater は 12.0.0 据え置き)→ 機械受入 →
...
- **案B(更新のみ)**: 12.1.1 へ更新し回避策は 4 層とも全維持。撤去は次版へ分離。diff 最小・
  ただしリフレクション依存の保守負債と「契約に嘘が残る」状態(M-HARNESS-015 の②が実質不要)が続く。
- **案C(見送り)= 不採用推奨**: 12.0.4 据え置き。自ら報告した修正の恩恵(#21770/#21688)を取り逃し、
  以後の上流追随コストが積み上がる。
...
- **gate①(裁定)= 必須・未**: maintainer 裁定事項は 3 点:
  1. **更新範囲**: 案A(更新+段階撤去)/案B(更新のみ)/案C(見送り・不採用推奨)。
     ItemsRepeater 12.0.0 据え置きの可否も併せて。
  2. **FailFast 監視の撤去可否と判断基準**: 「pin 赤化」だけで撤去してよいか、
     **挙動裏取り(cleanup 例外注入)まで求めるか**(案A は後者を採る)。
  3. **ECO-082 の扱い**: 本 ECO で再測のみ行い記録へ留めるか、束ねて再開するか。
