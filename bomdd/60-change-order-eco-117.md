...
## §9 クローズ(2026-07-20 golden 合格)

### 9.1 実機確認(maintainer)

§6 提示の基準 3 点で合格:

1. ゴミ箱ポップアップ(画像タブ)を件数多めで開き、**最下端まで送って最後のカード行が完全に見える**+余白が従来どおり
2. 作業タブのゴミ箱も同様
3. タグタブのビュー一覧/タグパレットの見た目が破綻していない

lint 自体は機械証拠(是正前 4 件赤→0 件 pin の緑転)で検収(ECO-105/111 前例)。

### 9.2 再発防止

- **CP-AXAML-LINT-117 新設**(automated): src 全 `.axaml` の禁止則ゼロ基線を恒久検査。
  潜伏実績(遡及掃射なしで 8 サイトが潜伏)と理論限界(現存 0 件を grep 確認済み)を行内に刻印。
- **CP-UI-G1 へゴミ箱面の到達性観点を刻印**(ECO-116 刻印の適用範囲拡張)。
- **K-AVALONIA 刻印**(fix 時): 禁止則+実測法則+lint 紐づけ+時系列訂正済みの教訓。
- **M4 同期= 不要**: surface 新設も挙動仕様変更もなし(余白の持ち方のみ・ECO-116/038 と同判定)。

### 9.3 教訓

1. **法則発見時は遡及掃射(read-across)+機械検査(lint)まで込みで初めて規約になる。**
   注意書き(2026-07-01/02)は将来のコードに向けた防御であり、既に書かれた 8 サイトには
   届かなかった — 発見の瞬間に grep 一発の遡及掃射をしていれば全サイトがその日に見つかった。
   ECO-107 教訓 1(lint 限界宣言には掃射手段紐づけ)と ECO-003 readacross_lesson
   (ゲート追加と read-across は別アクション=前者が将来防止/後者が過去出荷の掃討)の**合流形**。
   BomDD 昇格候補: 「知識刻印(K-BOM/コメント)の受入条件に遡及掃射の実施証拠を含める」。
2. **R8 の裏取り対象には「fix が語る歴史」も含まれる。**
   本 ECO のスコープ内欠陥は コードでなく**因果の記述**だった(「注意書きの後に書かれた」=
   git 遡りで反証された時系列誤り)。ECO-114 教訓 2(fix の依拠する事実主張の裏取り)・
   ECO-116 教訓 3(レビュー所見も一次資料でない)に続く系列で、**歴史主張は git が常に正**
   (N=3 =昇格判定 DUE)。誤った因果を K-BOM に刻印すると、将来の診断が誤った前提から
   始まる — 台帳の事実性はコードの正しさと同格の受入対象。
3. **法則の三点確認は残存サイトの実害予測を無料にする。**
   ECO-116 の 2 実測(16/14px)に本 ECO の 1 実測(18px)を加えた三点で
   「到達不能量= Padding.Top」が線形則として固まり、TagsTab 2 面(Top=0)の
   「症状なし」を是正前に確信をもって予測できた(実測でも追認)。
   欠陥クラスの是正では**法則を先に数値で固める**と、掃討対象の優先度付けが計算になる。
