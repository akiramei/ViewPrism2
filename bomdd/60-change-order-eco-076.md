# ECO-076(applied / クローズ済み 2026-07-13): 取り込みウィザード stepper の B-3・B-4 可視化 — CAD mock 改版への追随
...
- fix 済みビルドで **B-3/B-4 とも stepper 表示 OK** の報告(§7.5 項目 2・3)。
- B-2(1-2=青・3-4=灰・接続線 1-2 間のみ青)と en ラベル追随(§7.5 項目 1・5 の一部)は、
  初回所見の切り分け時に提出されたスクリーンショット(en UI)で確認済み。
- 初回 golden の所見「B-3/B-4 で stepper 非表示」は**偽不合格**と切り分け(git 実測):
  fix `542ef87`(初回 `2ceb938`。BomDD-ECO-Fix trailer が中間段落で git trailer 非認識=
  validate_bom E15 検出のため、accept 時に message のみ amend・内容同一)は worktree ブランチ
  `claude/sleepy-blackwell-9a1215` のみに存在し、
  実行された main 作業コピーは `a96a782`(起票前)・App.exe は 7/12 ビルド。
  B-2 が新旧同一見え(stepper は旧契約でも B-2 に表示)のため、B-2 だけでは
  ビルド取り違えに気づけない構造だった。製品欠陥ではない。

### 8.2 再発防止の場所

- CP-UI-G13(33-control-plan.yaml): stepper 全可視面契約+VC-1〜VC-4 並置+潜伏実績 2 件
  (①検査器の pin は CAD 改版で契約ごと反転する ②golden はビルド出自確認から)。
- 機械側: GfPackageVisualParityTests の VC probe 4 本(B-2 状態遷移/B-3 可視+3 点灯/
  B-4 緑チェック+全区間青/B-1 非表示)が恒久 pin。

### 8.3 教訓

**旧実装に忠実な検査器は、上流(CAD)改版時に「正しさの向き」ごと反転する。**
ECO-038 の教訓「派生プロパティ通知漏れは XAML バインド名基準で検査」が実装→検査の向きなら、
本 ECO は検査→契約の向き: pin テストは「現契約の忠実な固定」であって「恒久の正」ではなく、
CAD 改版追随 ECO では (a) 反転する pin の特定と改訂、(b) 新契約の VC probe 先行生成(R7)、
(c) CP characteristic の契約記述改訂、の 3 点を**同一 diff で**行う(どれかが残ると旧契約が
台帳に生き残り、次の追随で「どちらが正か」の考古学が要る)。また golden 運用の read-across:
**見た目が新旧同一の面(B-2)は「新ビルドが動いている」証拠にならない** — 挙動が変わる面
(B-3/B-4)で検証する前に、実行バイナリの出自(fix コミット含有・ビルド時刻)を確認する。
worktree ブランチ運用では fix が main の実行環境に自動では届かない点が構造原因
(ECO-060 の「入口スキル境界」と同様、環境の境界も明示確認が要る)。
