...
- **golden**: approved(2026-07-19 maintainer 実機・26 万件ビュー基準 6 点): ①全 5 モードの開始が
  即時 ②終了も即時 ③視覚不変(チェック/バッジ/白✓/整理マーカー・閲覧復帰後のドット欠落なし)
  ④チップ・件数・ソート不変 ⑤**挙動差分=モード遷移でスクロール位置維持も許容承認**(ECO-110
  契約と同方向の改善・CP-UI-G1 刻印) ⑥作業タブも同様。
- **再発防止**: CP-UI-G1 へモード遷移の計算量契約+潜伏実績(ECO-113 教訓の即日実証)+承認済み
  挙動差分を刻印。E-UI-BROWSE-022 へ invariant 追加(M4)。機械側= インスタンス同一性 probe
  (ImageTab 4+WorkTab 1)で pin。
- **M4 同期**: E-BOM invariant のみ(視覚・意味論不変の計算量是正=ECO-113 と同型。REQ 新設なし)。
- **残課題**: タグ編集パネル操作系(TabCurrent/TabAdd/ClickAddRow 等)の全面 Recompute 残余=
  同族だが別経路・症状未観測(51-cheat-log 記帳)。

### 教訓

1. **教訓の即日実証=「経路棚卸し」は起票様式へ昇格すべき**: ECO-113 教訓 1(規模系 NFR は経路ごとに
   独立して破れる)を書いた当日に、予見リストに挙げた「モード遷移」経路がそのまま顕在化した。
   N=4(064 起動→062 検索→113 選択→114 モード遷移)。大規模 fixture の性能所見を受けた ECO は、
   起票時に**同規模へ触る操作経路の棚卸し表**(クリック/モード遷移/パネル操作/スクロール/ナビ)を
   §3 に含める様式を提案する — 今回は棚卸しがあれば 113 と 114 は一度の診断で並べられた。
   **BomDD 昇格候補(N=4・様式提案つき)**。
2. **R8 は「自分の設計根拠」も検証対象**: WorkTab「選択保持」という私の実装時仮定は実コードと
   矛盾していた(ResetOrganizeState 末尾の Clear を見落とし)。fresh context が git show まで遡って
   反証した — 独立レビューには「diff の欠陥」だけでなく**「fix が依拠した事実主張」の裏取り**を
   明示的に含めるべき(死んだ一般性=誤った仮定に基づく複雑化を防いだ実例)。
3. **是正は既存コメントの根拠を偽にする**: 構造変更(再構築→その場更新)で、旧構造を根拠に書かれた
   コメント 3 箇所(是正対象外のファイル含む)が偽になった。BomDD はコメントが契約根拠を運ぶため、
   構造変更 fix では「旧構造を前提に書かれた記述」の grep 掃射(今回= "Recompute" 言及)を
   R8 観点に含める。
