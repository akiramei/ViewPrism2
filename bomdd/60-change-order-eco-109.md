...
- status: applied(2026-07-18 golden 合格・クローズ)
...
## §7 クローズ(2026-07-18 golden 合格)

### 7.1 実機確認(maintainer)

- 並び替えメニュー(SORT-menu.png 並置=VC-FL-1 ①〜⑥)・ツールバー(TB-grid.png=VC-FL-2 ①〜⑤)・
  リスト/タイル(VC-FL-3/4)・未ソート状態(バッジ「なし」・チップなし)・リスト⇄アイコン切替の
  メニュー閉+ソート状態共有・作業タブ同型面 — すべて合格。
- golden 1 巡目所見= **GF-109-01**(方向矢印が行末でなく種別チップ直後)→ 距離実測 probe(赤)→
  `HorizontalContentAlignment=Stretch` 是正(`d8c3962`)→ 2 巡目で合格。
- **裁定 3 件=許容承認**: ①タグ色ドットの halo(mock boxShadow 20%α)非転写 ②popupMenu 枠色
  CardBorderBrush #E8EBF0(mock #e3e7ee) ③メニュー見出し/非アクティブラベル文字色=既存リソース維持。
  以後の golden で差分扱いしない(CP-UI-G1 へ刻印済み)。

### 7.2 再発防止

- CP-UI-G1 characteristic へ ECO-109 観点を刻印: VC-FL-1〜4 の転写内容・同型 2 面同時追随・
  GF-109-01(整列は距離の実測で検査)・許容差分 3+2 件。
- 機械側 pin= GfSortMenuVisualParityTests(VC-FL-1/2 両面+行末距離 probe)。

### 7.3 教訓

1. **整列の検査は「存在」でなく「距離」**(GF-109-01): 端寄せ・中央寄せは要素の存在検査や回転検査では
   拾えない。Left 系コンテンツ整列の中の `Dock=Right` は自然幅縮退で「内容の右端」へ退化し、probe と
   R7 並置の双方が沈黙した。既存教訓との read-across= GF-103-01(Bounds 中心比較は Stretch を素通り=
   縦中央の同型)と対になる**横方向の同型**。「装飾 3 レイヤー(ECO-103 教訓 3)」の「整列」レイヤーは
   距離実測が正規の検査手段 — BomDD 昇格候補(§7.4)。
2. **許容差分は面ごとに再裁定される**: タグタブの種別チップ言語(シンプル=灰)と本面(シンプル=紫)は
   CAD 内で意図的に異なる(kindChip は「基本」が灰を占有)。共有リソース(ChipSimpleBg)の名前意味論と
   面の色言語が乖離する場合、面ローカルの直値+根拠コメントで保持し、リソース転用で名前を汚さない。
3. **公開監査は fixture 名にも効く**(運用メモ): ハーネスのシード名が third-party-title 検出で
   commit ブロック=fail-closed が機能。CAD mock の固有名は公開リポでは一般名へ置換して転写する。

### 7.4 BomDD 昇格候補

教訓 1(整列検査は距離実測 — 存在/回転検査と R7 目視並置の双方が整列退化に対して盲目)は
ViewPrism2 固有でない一般形(GF-103-01 の縦・GF-109-01 の横で N=2)。方法論リポ(`../BomDD`)への
昇格候補として記録(昇格は別リポ・別オーダー。既存の「Stretch 上寄り N=3=昇格判定 DUE」系列と
同族の検査粒度ファミリー)。

## §8 残ゲート(fix 後・クローズ済み=経緯記録)
