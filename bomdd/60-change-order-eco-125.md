# ECO-125 — Recompute 結合点の予防棚卸し — 列挙軸を「経路種別」から「結合点」へ(ECO-124 教訓1 の実行)(applied)
...
- **golden approved(maintainer 実機・基準 5 点)**: ①候補追加即時+意味論 ②続ける即時+リセット
  ③別コレクションスキャン開始の非干渉 ④表示中再スキャンの取込順固定回帰(ECO-060)⑤整理実行系回帰。
- **M4**: 不要(E-BOM invariant は fix 時刻印済み・surface/挙動仕様変更なし)。
- **再発防止**: **CP-RECOMPUTE-LINT-125 新設**(automated・台帳= コード内 Ledger が正本)+
  CP-UI-G1 刻印(B 3 経路の母集合非通過・「予防発見」の潜伏実績つき)。
- **教訓(read-across 明記)**:
  1. **予防棚卸しは事後観測の起票コストを一括置換する**。26 万件経路は 7 ECO(それぞれ症状観測→
     診断→是正)を要したが、結合点軸の全数分類 1 回で残余 3 サイトを症状ゼロのまま処置した。
     「破れは経路単位・棚卸しは軸単位」(ECO-124 教訓1)の解= **呼ぶ側の意味論(経路)でなく
     呼ばれる側の結合(結合点)を数える**と谷間が構造的に出ない。26 万件経路ファミリー(N=7+予防 3)の
     BomDD 昇格候補を「事後→予防の転換」まで含む完成形へ。
  2. **第一印象を結論にしない(ECO-107 教訓2)の再実証**: 起票時第一印象「B 既知 0」を全数調査が
     覆して 3 サイト発見。うち B-1 は「同一意味論・別入口」の面内非対称(グリッドクリック側だけ
     ECO-113 で是正済み)= **是正の悉皆は操作の意味論単位で切る**(入口単位だと取り残す)。
  3. **lint 台帳の正本をコードへ同居させると台帳ドリフトが機械的に不能になる**。分類根拠を
     Ledger(lint 内)に置いた= 台帳と検査が同一物で、乖離の余地がない(ECO-122 写像の
     「出典コメント+機械 pin」と同系の一元化)。
  4. R8 の貢献 2 件は本 ECO の型の限界を補った: 所見3(部分更新が「母集合不変」でも**別の状態機械
     (ソート実現)の変異点**を見落とす= 不変条件は次元ごとに検査)・所見6(直呼び lint の死角=
     デリゲート間接呼び出しに B 相当が残る= 検出限界の宣言と人力レビューのセット= ECO-122 教訓2 同型)。
