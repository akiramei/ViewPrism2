# Change Order — ECO-108(applied): メインタブ/設定面の常駐値 18 サイトの言語追随(ECO-107 lint③ の deferred 層)
...
## 7. クローズ(2026-07-17・golden approved)

**maintainer 実機承認(基準 3 点)**: ①画像タブ= 列見出し(Name/Size/Modified Date)・ソート
ポップオーバー候補・Basic チップ・件数ラベルの en 追随+ja 往復(所見スクリーンショットの再現手順)
②作業タブ= 件数・チップヒントも追随(従来の列見出しに加え)③設定= スナップショット要約の追随 —
全点合格。

**再発防止の場所**: CP-I18N-010 ⑦(対称化・ヘルパ返却値の死角・既知限界の根拠残置)/
CpI18n010TabVmLabelTests(画像タブ probe= GF-079-01 の WorkTab probe と対)/
CpI18n010AssetLintTests allowlist(deferred 層解消=全エントリが根拠つき精査済みに)。

**教訓**(1 段落・既存教訓との関係):

**片面だけの是正は「面間非対称」として次の潜伏を作る — 同型面へ read-across してから閉じる**:
GF-079-01 は WorkTab に Rebuild 対を入れたが、同じ焼き込み構造を持つ ImageTab へ展開されず、
4 日後に実機で非対称(作業タブ=英語・画像タブ=日本語)として顕在化した。同型構造を持つ面が
複数あるとき、1 面の是正は残る面の「未是正の証明」でもある — 是正時に同型面を列挙し、適用または
非適用の根拠を残す(ECO-088/089 の「同型残存の read-across 閉包」= playbook §8.2 と同じ規律の
言語追随版であり、既に昇格済みの原則が水平展開の手を離れて再発した形。N=2)。付随の教訓=
VM 直書き禁止の検査は「文字列を返すヘルパ」に及んでいなかった(XAML lint= axaml のみ・lint③=
代入のみ)— 検査の谷間は「返却値」にもある(KindChipLabel が実例・CP-I18N-010 ⑦に刻印)。

**ECO-107 との連動**: lint③ allowlist の deferred 層(c)が解消し、全 34 エントリ由来の残余が
根拠つき精査済み(モーダル 15・射影 1・再解決経路 13・既知限界 2)となった=悉皆処置完了。

**スコープ外の後続**: Core result.Message の i18n 化(_undoNote×2 の既知限界・将来候補)・
flaky 一括起票候補(ECO064+SaveBar TestContext)。
