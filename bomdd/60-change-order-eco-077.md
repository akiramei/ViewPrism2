# ECO-077(staged): バックアップ・移送の入口を設定 ▸ データとバックアップへ集約(SS-001 再裁定=E-1 追随)
...
- gate①: §4.2(取り込み先コレクションの決定方法)の裁定 → 採用案の ViewPrismUI 正典化が
  fix の先行条件。あわせて 0 件時 placeholder(§4.1)の設計者適用可否。
- gate②: golden(CP-UI-G12/G13 の E-1 観点・操作手順は fix 時に §8 として確定)。
