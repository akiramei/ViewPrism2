# ECO-077(applied / クローズ済み 2026-07-13): バックアップ・移送の入口を設定 ▸ データとバックアップへ集約(SS-001 再裁定=E-1 追随)

- 起票: 2026-07-13(maintainer 指示・ViewPrismUI SS-001 再裁定への追随)
- 種別: 既存機能の設計変更追随(CAD 上流裁定の転写。機能の実体=A層/B層エンジンは不変)
- 状態: applied(2026-07-13 gate②合格=§11)
...
## 11. クローズ(2026-07-13 gate②合格)

### 11.1 実機確認(maintainer)

- §10 の全 7 項目合格: E-1 並置(行カード 3 枚・ナビ選択・節見出し・注記)/サマリ L8+0 件
  placeholder/B-1 内選択+既定出力先追随/B-2 取り込み先未選択で「次へ」不活性→選択で活性→
  B-3/B-4 回帰なし/⋯誘導 1 項目(実体なし)→節選択オープン/ja・en/既存面回帰なし。
- 記録済み差分 D1(擬似タイトルバー)・D2(幅 720)・D3(閉じるボタン残置)・D4(誘導グリフ=歯車)
  =すべて許容承認。

### 11.2 再発防止(恒久化の所在)

- CP-UI-G12/G13 characteristic へ E-1 入口観点を fix 時に刻印済み。承認された許容差分は
  CP tolerance へ恒久記録(G12: 幅 720/閉じる残置/0 件 placeholder、G13: 誘導グリフ=歯車)
  = 将来の golden で再審理しない。
- 機械側 pin: GfEntryE1VisualParityTests(VC-5〜8)+CpUiEco077EntryTests(B-1/B-2 内選択・
  誘導コマンド)。入口の正本=CAD snapshot_export_import(4787b03+05080e1)+仕様 §2.13.5/§2.14.6。
- SS-004(0 件表記)は暫定 placeholder のまま存続 — 正式裁定時に追随(CAD E-1 状態表に記録済み)。

### 11.3 教訓

- **入口(導線)の集約は「旧入口が供給していた文脈パラメータの棚卸し」を影響分析の一次元として
  含める**: 三点メニュー(コレクション文脈)→設定(文脈なし)への集約で、文脈が暗黙に運んでいた
  「書き出し対象」「取り込み先」が孤児化した。CAD 改版は書き出し側の後継(B-1 内選択)を明記した
  一方、取り込み側は沈黙し、さらに旧 V1 差分裁定の代替手順(「別コレクションの三点メニューから
  取り込み直し」=32-mbom:823)も同じ文脈に依存して静かに失効していた。**導線変更の diff は
  UI 要素の付け替えに見えて、実体は「パラメータ供給経路の変更」**であり、①旧導線が束縛していた
  引数の全列挙 ②その引数に依存する既裁定(沈黙次元・許容差分)の失効チェック、の 2 つを
  機械的に行うべき。ECO-074 教訓「機能族の先行裁定 read-across」の姉妹形=**導線変更時は
  文脈依存裁定の失効 read-across**。方法論昇格候補。
- R7 セルフゴールデンの有効性 2 例目(ECO-076 に続く): fix 内の並置が B-1 ComboBox の
  既定枠の浮き(mock=カードに溶けた選択)を golden 前に検出・是正。転写漏れが golden へ
  漏れる前に製造側で止まった。
