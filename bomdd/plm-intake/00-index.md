...
> 状態語彙: `proposed / assigned / applied / verified / rejected / not-exported`。
>
> 注記(ECO-PROJ-003/004 = `applied`): ViewPrism2 成果物側の是正は適用済。**BOM-DD PLM の bomdd 再同期による Finding 解消の確認は未実施**(`verified` への昇格は再同期後)。各票の検証手順を参照。
...
>
> 留意: PLM 側が `unused-template` / `candidate(未採用)` をまだ準拠判定で解釈しない場合、再同期後も該当 finding が残り得る。その是正は ViewPrism2 成果物側ではなく **BOM-DD PLM の判定ロジック側**で行う(別リポ・別タスク)。
>
