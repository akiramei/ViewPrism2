...
- status: applied(2026-07-18 機械証拠検収・クローズ)
...
## §8 クローズ(2026-07-18・機械証拠検収)

- **検収**: golden n/a(ECO-090/105 前例)。機械証拠= §7(AVLN5001 フルビルド 1→0 件・
  警告 0 個・機械受入 4 点全緑)を maintainer が確認し accept 指示(2026-07-18)。
- **再発防止**: K-AVALONIA(31-kbom)へ実装規約を刻印 — 廃止 API は警告顕在化時点で後継へ
  追随し AVLN 系警告 0 を維持・**検証はフルビルド(--no-incremental)で行う**(インクリメンタル
  ビルドは XAML 再コンパイルなしで警告を非表示にする実績つき)。

### 教訓

**警告は数えなければ資産でなく雑音になる。** 本件の滞留は二重のマスクによる: ①機械受入基準
「0 error」が警告を数えない ②インクリメンタルビルドが XAML 警告自体を非表示にする(フルビルド
時のみ再現)。恒常警告が 1 件でも残ると「警告が出ている状態が正常」という基準線汚染が生じ、
次の新規警告の検出力を失う — ECO-107 教訓「資産負債は錯覚のマスク」の警告版 read-across。
後続候補(policy 変更=本 ECO 外): 機械受入へ「フルビルド警告 0」を恒久基準として加えるかは
maintainer 裁定事項(現状は K-AVALONIA 規約+本 ECO の実績記録で運用)。
