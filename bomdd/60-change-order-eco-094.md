...
## 8. クローズ(2026-07-15 golden 合格・applied)

**gate② 合格(maintainer 実機)**: 両タブの視覚・挙動不変を再承認 — チップ意匠(色ドット/件数
バッジ/ナビ矢印/未定義値破線+バッジ)・47 件級の最大 2 行+「ほか N 件」+ポップオーバー
(検索/選択/Escape)・作業タブへの意匠漏れなし・幅変更再計算・en 切替。

**M4 同期(accept 時)**: ①35-dsbom へ **SC-CHIPSTRIP-001** 登録(realized・visual/lifecycle
契約・dry_gate 明記)+shared_component カタログ追加 ②30-ebom E-UI-AXIS-NAV-040/
E-UI-WORKSPACE-043 の **ECO-090 同期宣言を部品移管完了へ更新**(宣言どおりの移管=
「その時点で部品側の契約へ移管する」の実行)③CP-CHIPWRAP-088 へ部品化観点+CpUi094 を拡張。

**再発防止**: 構造 probe(CpUi094=両タブのチップ行が共有部品で実現)が再コピペ化への後退を
恒久検出。CP-CHIPWRAP-088 に刻印。

**教訓**: ①「isolate now, DRY later」の**later を裁定で日付にする** — ECO-091 §7 が「契約一致
確定後の別 ECO」と先出ししたため、契約収束(091/092/093 golden)の翌日に摩擦ゼロで抽出できた。
暫定統制(ECO-090 の read-across 必須)は DRY までの橋であり、部品化で**構造保証へ置換して
宣言を畳む**ところまでが一式(SC-COLUMN-PICKER-001=ECO-025 と同型の 2 例目=様式として確立)。
②**テンプレート統一は「richer 版へ寄せて不活性化」**が視覚不変と単一実装を両立する — 差分側
(未定義値枠/ナビ chevron)を bool で不活性にできるか、消費面の工場(Neutral/Colored)を実測
してから決める。probe は「不活性が漏れない」pin(CpUi094)で封止する。
