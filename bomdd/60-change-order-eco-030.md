...
> **処方変更(2026-07-03 実装検証)**: 起票の前提「FMEA の定義サイトが存在しない」は**誤認**だった。
> 32-mbom.yaml にトップレベル `fmea:` 構造表(36定義・`{id, unit, failure_mode, effect, targeted_by}`)が
> **既に実在**し、参照35 ID は全て表に解決可能(dangling ゼロ)。warn 43件の真因は ref-v0.3 の定義サイト規定
> (33 のみ=BomDD-Plm 自リポの配置だけを見た書き癖)であり、**「実物が正」6回目の適用として ref-v0.5**
> (32/33 両対応)で解消した。本リポの変更は FMEA-037 の逆参照1行の同期のみ(33 への移設・転記は不要)。
> S-19 裁定の「FMEA 構造表が無い実態の可視化」も誤認として訂正する(表は実在・スキーマが場所を知らなかった)。

- **type**: 台帳構造化(doc-only — 散在する FMEA 言及を機械可読な表へ)→ **実際: スキーマ側是正(ref-v0.5)+逆参照1行同期**
- **status**: applied(処方変更の上で適用 2026-07-03)
- **golden**: n/a
