...
|---|---|---|---|---|
| 整理トレイパネル | 右ペイン容器 + ヘッダ(整理) + selection/段落 | **extend** | タグ編集パネル容器(CMP-0030)と同型 + 整理見出し variant | 素パネルに退化(タグ編集と不揃い) |
| マージ先カード | 残す1枚の強調カード(枠強調 + ラベル) | **extend** | グリッドカード(CMP-0027)+ accent 枠 variant | 「残す1枚」が判別不能 |
| 整理対象カード | 削除対象カード + ×(外す)+「マージ先にする」 | **new** | 新規(カード + 行内アクション) | 整理対象とマージ先の区別が死ぬ |
| 似た画像を探す検索フォーム | 方式切替(条件/類似)+ 閾値スライダー + 条件入力 | **new** | 新規(セグメント + Slider + 条件行)。閾値 = E-UI-SIMILARITY-035 | 素フォームに退化・2方式の手触りが死ぬ |
| 検索結果カード | 候補サムネ + **一致率バッジ** +「整理対象に追加」状態 | **new** | 新規(グリッドカード + %バッジ + 追加済 variant)。サムネ = E-THUMB-020 | 一致率/追加状態が不明 |
| タグ統合トグル | 「マージ時にタグを含める」チェック | **extend** | 既存チェック/トグル + 説明文 | 素チェックに退化 |
| 完了状態カード | 「統合しました」+ 結果件数 +「取り消す」「別の整理を続ける」 | **new** | 新規(結果サマリ + アクション 2 つ) | 完了/取り消し導線が不明瞭 |
| 整理を終了(ツールバー) | active トグル / 終了ボタン | **covered** | `Button.toolbarBtn`(.toggle)= タグ編集と同型 | 素ボタンに退化 |

ロールアップ(整理トレイ分): **covered 1 / extend 3 / new 4**。`new` 4 部品(整理対象カード・検索フォーム・検索結果カード・完了状態カード)は製造前に Components.axaml へ追加(DS1)。

> **ゲート(ECO-014)**: 上記 `new`/`extend` は整理モード製造フェーズで Components.axaml に定義してから surface へ適用する。本表は候補(未実証)。実装着手前に `new` を製造計画へ載せること(`design_system_part_missing` 回避)。
