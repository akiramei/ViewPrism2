...
public sealed class CpDb006Tests : IDisposable
{
    ...
    /// <summary>
    /// v0(初版)DDL のフィクスチャ(Run 1 当時の DatabaseSchema.LatestDdl の複製)。
    /// 以後のマイグレーション(001-views-description 等)適用で最新スキーマと同値になることを検査する。
    /// </summary>
    private const string V0InitialDdl = """
    ...
    public void v0DBに全マイグレーション適用で新規DBとスキーマ同値()
    {
        ...
        ExecuteRaw(v0Path, V0InitialDdl);
