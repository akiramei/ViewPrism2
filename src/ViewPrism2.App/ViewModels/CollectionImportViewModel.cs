...
public sealed partial class CollectionImportViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(VerifyOk), nameof(CanProceed))]
    private string? _verifyError;

    public bool VerifyOk => Header is not null && VerifyError is null;

    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(VerifyOk), nameof(CanProceed), nameof(HeaderCollectionName), nameof(HeaderImageCount), nameof(HeaderTagCount), nameof(HeaderCreatedAt), nameof(HeaderAppVersion))]
