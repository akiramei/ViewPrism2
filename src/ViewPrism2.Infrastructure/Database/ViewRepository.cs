...
public sealed class ViewRepository : IViewRepository
{
    private const string SelectViewColumns = """
        SELECT id AS Id, name AS Name, is_favorite AS IsFavorite, sort_field AS SortField,
               sort_direction AS SortDirection, display_columns AS DisplayColumns,
    ...
    public Task AddAsync(View view)
    {
        ...
        return _db.RunAsync(conn => conn.ExecuteAsync("""
            INSERT INTO views (id, name, is_favorite, sort_field, sort_direction, display_columns, home_tag_id, modified_at)
            VALUES (@Id, @Name, @IsFavorite, @SortField, @SortDirection, @DisplayColumns, @HomeTagId, @ModifiedAt)
            """,
            new
            {
                view.Id,
    ...
    public Task UpdateAsync(View view)
    {
        ...
        return _db.RunAsync(conn => conn.ExecuteAsync("""
            UPDATE views
            SET name = @Name, is_favorite = @IsFavorite, sort_field = @SortField,
    ...
    private sealed record ViewRow(
        string Id, string Name, long IsFavorite, string SortField, string SortDirection,
