...
- **一次資料の不改変=golden 承認済みが対象**: golden で承認された `資料/<画面>/` の mock は編集しない。
  **差し替えは新納品 HTML**(新ファイル)で行う(ViewPrismUI CLAUDE.md 禁止事項)。ただし **golden 前の
  AI 起草 Provisional ドラフトは、同一レビューサイクル内なら同一 delivery の版として in-place 改訂してよい**
  (golden が凍結点)。混同を避けるため、承認前は「v2」等の同一ファイルを更新し、版の乱立を作らない。
