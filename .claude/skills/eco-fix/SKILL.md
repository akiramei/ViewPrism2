---
name: eco-fix
description: 起票済み ECO の是正実施。プローブ先行(是正前不合格の実測裏取り)→最小是正→機械受入→セルフゴールデン(R7・UI fix は CAD captures 並置で転写漏れ 0)→セルフレビュー(R8・src fix は fix diff の独立レビューで未処置スコープ内所見 0)→fix コミットまで行い、golden 合格基準(human gate)を提示して停止する。起票(/eco-file)がまだの案件には使わない。
---
...
3.7. **セルフレビュー(R8・src に触れた fix は必須)**: golden 提示(golden n/a の ECO では
   accept 依頼)の**前**に、fix diff への**独立コードレビュー**を実施する。独立性の担保=
   fix を書いたコンテキストと別の fresh context(/code-review 相当の subagent を diff に対して
   起動。裁量で Codex 二次意見を追加)。所見は**全列挙**して処置を分類する:
   - **スコープ内欠陥** → 本 ECO 内でプローブ先行是正(手順 1 へ戻る)。
   - **スコープ外** → R3(分離起票 or 51-cheat-log)の二択。
   **未処置のスコープ内所見 0 になるまで停止点に進まない。**
   レビュー結果(所見全数+処置分類・所見 0 ならその旨)は手順 4 の実施記録へ含める。
   doc-only・機械的 1 行変更(ECO-111 型)は「R8 対象外」を宣言して省略可(R7 と同型)。
   背景: 2026-07-17 の maintainer レビュー 2 セッションが計 6 所見→4 ECO(101/102/104/105)を
   生んだ実績=潜伏 0〜1 日の diff 精読で先取り可能な所見(accept 後レビューは手戻り最大)。
